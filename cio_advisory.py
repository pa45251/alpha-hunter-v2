...
def _advisory_for_row(row: pd.Series) -> dict[str, object]:
    ...
    if source_backed and route == "STOCK_ALPHA_RESEARCH":
        if reaction in ENTRY_FRIENDLY_STATES:
            return {"advisory_action": "BUY_BIAS_STOCK", "advisory_confidence": "HIGH" if reaction in {"CONFIRMING", "PULLBACK"} else "MEDIUM", "preferred_exposure": "STOCK", "advisory_rationale": "Active driver, source-backed company edge, and a non-extended reaction state support a positive stock bias."}
        if reaction == "PERSISTENT":
            return {"advisory_action": "HOLD_BIAS", "advisory_confidence": "MEDIUM", "preferred_exposure": "STOCK", "advisory_rationale": "The thesis is confirmed, but more information may already be priced; prefer hold or a better entry over chasing."}

    if not source_backed and direct:
        if reaction in ENTRY_FRIENDLY_STATES:
            return {"advisory_action": "PROVISIONAL_BUY_BIAS_STOCK", "advisory_confidence": "LOW", "preferred_exposure": "STOCK_RESEARCH_ONLY", "advisory_rationale": "Active driver and strong structural linkage create a favorable hypothesis, but company-level source backing is still missing."}
        if reaction == "PERSISTENT":
...
def build_advisory_packet(advisory: pd.DataFrame, run_id: str) -> dict:
    ...
    return {
        ...
        "decision_rule": "Always separate recommendation from execution permission. Active causal evidence may support a directional bias even when execution remains blocked. Trend evidence is exposed independently from entry reaction state; price still cannot create causality.",
    }
