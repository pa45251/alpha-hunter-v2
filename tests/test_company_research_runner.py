...
def test_transport_failure_remains_terminal_without_becoming_investment_unknown(tmp_path):
    ...
    def forbidden(*args): raise AssertionError('No document means no model call')
...
def test_execution_failure_is_not_cached_but_success_is(tmp_path):
    ...
    h = dict(run_id='run', company_research_targets=[target]); p = dict(company_targets=[]); calls = []
    def flaky(task, sources, shared, key, logs):
        calls.append(key)
        if len(calls) == 1: return None, ('TRANSPORT_FAILED', 'CLI_EXIT_1')
    ...
    assert len(calls) == 2 and second['company_research_coverage'][0]['status'] == 'UNKNOWN_AFTER_RESEARCH'
    third = run(h,p,cache,tmp_path/'logs',flaky)
    assert len(calls) == 2 and third['execution_ledger'][0]['cache_hit'] is True
...
def test_isolated_task_binds_missing_identity_and_run_metadata(tmp_path):
    ...
    def call(*args): return dict(company_research_coverage=[dict(status='UNKNOWN_AFTER_RESEARCH', reason='Missing fact')], company_opportunities=[]), None
    row = run(h, {}, tmp_path/'cache', tmp_path/'logs', call)['company_research_coverage'][0]
    assert (row['ticker'], row['driver_id'], row['thesis_id']) == ('1234', 'D', 'tid')


def test_unresolved_opportunity_is_downgraded_to_terminal_unknown(tmp_path):
    target = dict(ticker='1234', driver_id='D', thesis_id='tid', event_id='')
    h = dict(run_id='run', company_research_targets=[target])
    def call(*args):
        return dict(company_research_coverage=[dict(status='UNKNOWN_AFTER_RESEARCH', reason='Missing mechanism')],
                    company_opportunities=[dict(driver_state='UNKNOWN')]), None
...
def test_mapped_task_ignores_out_of_scope_exposure_proposal(tmp_path):
    target = dict(ticker='1234', driver_id='D', thesis_id='tid', event_id='')
    h = dict(run_id='run', company_research_targets=[target])
    def call(*args):
        return dict(company_research_coverage=[dict(status='UNKNOWN_AFTER_RESEARCH', reason='No transmission')],
                    exposure_resolutions=[dict(resolved_driver_id='OTHER')]), None
    result = run(h, {}, tmp_path/'cache', tmp_path/'logs', call)
    assert result['exposure_resolutions'] == []
...
def test_isolated_task_rejects_explicit_conflicting_identity(tmp_path):
    target = dict(ticker='1234', driver_id='D', thesis_id='tid', event_id='')
    h = dict(run_id='run', company_research_targets=[target])
    def call(*args): return dict(company_research_coverage=[dict(ticker='OTHER', status='UNKNOWN_AFTER_RESEARCH', reason='Missing')]), None
    result = run(h, {}, tmp_path/'cache', tmp_path/'logs', call)
...
def test_company_call_cannot_write_other_thesis(tmp_path):
    h = dict(run_id='run', company_research_targets=[dict(ticker='1234',driver_id='D')])
    def call(*args): return dict(company_research_coverage=[dict(ticker='OTHER',driver_id='D',status='SUPPORTED')]),None
    result=run(h,{},tmp_path/'cache',tmp_path/'logs',call)
    assert result['company_execution_failures'][0]['failure_code']=='SCHEMA_FAILED'
