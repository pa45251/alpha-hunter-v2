# Alpha Hunter — Action Board

- Run: `20260912T175525+0800-1432f3ae`
- Market session: `2026-09-11`
- Risk regime: **NORMAL**
- Causal evidence: `V3_AUTONOMOUS_RESEARCH`
- Core rule: find the anomaly -> understand WHY -> validate the same driver globally -> act only if price still offers a setup.

## Simple Opportunity Brief

### 1. 3044.TW 健鼎 — EARLY BUY

- **WHY:** Official August 2026 revenue increased 65.48% year over year and the issuer attributed the increase to customer order demand and higher shipments.
- **Driver:** A company-specific increase in customer orders and shipment volume is improving revenue before a broader global driver has been established.
- **Driver state:** DEVELOPING
- **Company transmission:** 健鼎's official monthly revenue disclosure specifically links the measured revenue increase to customer order demand and increased shipments, providing a company-specific operating mechanism without asserting a global taxonomy edge.
- **International confirmation:** Not required: The evidence identifies customer orders and shipments at 健鼎 but does not establish an industry-wide or international causal driver.
- **Relative:** LOCAL DRIVER
- **Regime:** NEUTRAL
- **Technical state:** Early strength / support recovery
- **Why price:** Observed 120-session resistance offers at least 2R before costs; support defines <=8% price risk
- **Entry:** 504–508
- **Invalidation:** Exit on loss of 470; thesis failure also invalidates
- **Add trigger:** Close above 521 with volume >=1.2x prior median; thesis still supported and recheck R/R
- **Main counter-evidence:** The nominated support and counter searches found no verifiable opposing company-specific disclosure; no global driver was established, so the local interpretation remains provisional.
- **Main risk:** Customer orders may be temporary, concentrated or delayed shipments rather than durable demand. The company is not classified as rate-sensitive because the evidence does not indicate exposure to real interest yields, long-end Treasury yields, liquidity or financing costs.
- **What would make us wrong:** Subsequent monthly revenue, order or shipment data showing that the August improvement reversed would falsify the provisional company-specific interpretation.
- **Initial size:** 35% of planned position; reassess before adding.
- Evidence: August 2026 monthly revenue was TWD 10224358 thousand, up 65.48% year over year and 4.10% month over month; the issuer states that customer order demand and shipments increased. [Official monthly revenue: 健鼎 11508](https://mopsfin.twse.com.tw/opendata/t187ap05_L.csv)

### 2. 3176.TWO 基亞 — WAIT

- **WHY:** Official August 2026 revenue increased and the issuer noted higher overseas revenue, but no specific project, order, product, clinical, regulatory or other company mechanism was verified to explain the change. No defensible provisional thesis can be formed.
- **Driver:** Unmapped opportunity — research WHY
- **Driver state:** UNVERIFIED
- **Company transmission:** Unverified
- **International confirmation:** Unverified — same-driver evidence required
- **Relative:** UNVERIFIED
- **Regime:** UNKNOWN
- **Technical state:** Early strength / support recovery
- **Why price:** Observed 120-session resistance offers at least 2R before costs; support defines <=8% price risk
- **Entry:** 36.1–36.5
- **Invalidation:** Exit on loss of 34.05; thesis failure also invalidates
- **Add trigger:** Close above 41 with volume >=1.2x prior median; thesis still supported and recheck R/R
- **Main counter-evidence:** Unverified
- **Main risk:** Exact company / driver transmission remains unverified; no entry recommendation
- **What would make us wrong:** A price-only story or weak company transmission

### 3. 4967.TW 十銓 — WAIT

- **WHY:** Official August 2026 revenue data shows higher shipment volume and selling prices, but the available evidence does not establish that NAND pricing, SSD/controller/module demand or inventory was the causal source, and no independent international evidence for the exact NAND/storage-cycle mechanism was verified.
- **Driver:** NAND / storage controller and module cycle
- **Driver state:** UNVERIFIED
- **Company transmission:** Unverified
- **International confirmation:** Unverified — same-driver evidence required
- **Relative:** UNVERIFIED
- **Regime:** UNKNOWN
- **Technical state:** Early strength / support recovery
- **Why price:** Observed 120-session resistance offers at least 2R before costs; support defines <=8% price risk
- **Entry:** 278.5–280.5
- **Invalidation:** Exit on loss of 268; thesis failure also invalidates
- **Add trigger:** Close above 294 with volume >=1.2x prior median; thesis still supported and recheck R/R
- **Main counter-evidence:** Unverified
- **Main risk:** Exact company / driver transmission remains unverified; no entry recommendation
- **What would make us wrong:** A price-only story or weak company transmission

### 4. 1609.TW 大亞 — WAIT

- **WHY:** Official August 2026 revenue increased, but the disclosure contains no copper-price, exchange-inventory, regional-premium, tariff or physical-tightness attribution. The measured revenue fact cannot by itself prove the nominated copper commodity trade-inventory mechanism.
- **Driver:** Copper commodity price / trade-inventory cycle
- **Driver state:** UNVERIFIED
- **Company transmission:** Unverified
- **International confirmation:** Unverified — same-driver evidence required
- **Relative:** UNVERIFIED
- **Regime:** UNKNOWN
- **Technical state:** Early strength / support recovery
- **Why price:** Observed 120-session resistance offers at least 2R before costs; support defines <=8% price risk
- **Entry:** 37.7–37.8
- **Invalidation:** Exit on loss of 36.5; thesis failure also invalidates
- **Add trigger:** Close above 38.95 with volume >=1.2x prior median; thesis still supported and recheck R/R
- **Main counter-evidence:** Unverified
- **Main risk:** Exact company / driver transmission remains unverified; no entry recommendation
- **What would make us wrong:** A price-only story or weak company transmission

### 5. 2636.TW 台驊控股 — WAIT

- **WHY:** WHY unresolved: obtain company evidence before taking risk
- **Driver:** UNMAPPED / WHY?
- **Driver state:** UNVERIFIED
- **Company transmission:** Unverified
- **International confirmation:** Unverified — same-driver evidence required
- **Relative:** UNVERIFIED
- **Regime:** UNKNOWN
- **Technical state:** Early strength / support recovery
- **Why price:** Observed 120-session resistance offers at least 2R before costs; support defines <=8% price risk
- **Entry:** 70.7–71.1
- **Invalidation:** Exit on loss of 67.5; thesis failure also invalidates
- **Add trigger:** Close above 79.5 with volume >=1.2x prior median; thesis still supported and recheck R/R
- **Main counter-evidence:** Unverified
- **Main risk:** Unverified causal interpretation
- **What would make us wrong:** A price-only story or weak company transmission

### 6. 4915.TW 致伸 — WAIT

- **WHY:** WHY unresolved: obtain company evidence before taking risk
- **Driver:** UNMAPPED / WHY?
- **Driver state:** UNVERIFIED
- **Company transmission:** Unverified
- **International confirmation:** Unverified — same-driver evidence required
- **Relative:** UNVERIFIED
- **Regime:** UNKNOWN
- **Technical state:** Early strength / support recovery
- **Why price:** Observed 120-session resistance offers at least 2R before costs; support defines <=8% price risk
- **Entry:** 62.6–62.7
- **Invalidation:** Exit on loss of 58.7; thesis failure also invalidates
- **Add trigger:** Close above 63.3 with volume >=1.2x prior median; thesis still supported and recheck R/R
- **Main counter-evidence:** Unverified
- **Main risk:** Unverified causal interpretation
- **What would make us wrong:** A price-only story or weak company transmission

### 7. 6150.TWO 撼訊 — WAIT

- **WHY:** WHY unresolved: obtain company evidence before taking risk
- **Driver:** UNMAPPED / WHY?
- **Driver state:** UNVERIFIED
- **Company transmission:** Unverified
- **International confirmation:** Unverified — same-driver evidence required
- **Relative:** UNVERIFIED
- **Regime:** UNKNOWN
- **Technical state:** Early strength / support recovery
- **Why price:** Observed 120-session resistance offers at least 2R before costs; support defines <=8% price risk
- **Entry:** 61.9–62.2
- **Invalidation:** Exit on loss of 58.3; thesis failure also invalidates
- **Add trigger:** Close above 64.2 with volume >=1.2x prior median; thesis still supported and recheck R/R
- **Main counter-evidence:** Unverified
- **Main risk:** Unverified causal interpretation
- **What would make us wrong:** A price-only story or weak company transmission

### 8. 4951.TWO 精拓科 — WAIT

- **WHY:** WHY unresolved: obtain company evidence before taking risk
- **Driver:** UNMAPPED / WHY?
- **Driver state:** UNVERIFIED
- **Company transmission:** Unverified
- **International confirmation:** Unverified — same-driver evidence required
- **Relative:** UNVERIFIED
- **Regime:** UNKNOWN
- **Technical state:** Confirmed breakout
- **Why price:** Observed 120-session resistance offers at least 2R before costs; support defines <=8% price risk
- **Entry:** 82.6–83
- **Invalidation:** Exit on loss of 76.6; thesis failure also invalidates
- **Add trigger:** Close above 83.5 with volume >=1.2x prior median; thesis still supported and recheck R/R
- **Main counter-evidence:** Unverified
- **Main risk:** Unverified causal interpretation
- **What would make us wrong:** A price-only story or weak company transmission

### 9. 5609.TWO 中菲行 — WAIT

- **WHY:** WHY unresolved: obtain company evidence before taking risk
- **Driver:** UNMAPPED / WHY?
- **Driver state:** UNVERIFIED
- **Company transmission:** Unverified
- **International confirmation:** Unverified — same-driver evidence required
- **Relative:** UNVERIFIED
- **Regime:** UNKNOWN
- **Technical state:** Early strength / support recovery
- **Why price:** Observed 120-session resistance offers at least 2R before costs; support defines <=8% price risk
- **Entry:** 78.7–78.8
- **Invalidation:** Exit on loss of 77.4; thesis failure also invalidates
- **Add trigger:** Close above 80.9 with volume >=1.2x prior median; thesis still supported and recheck R/R
- **Main counter-evidence:** Unverified
- **Main risk:** Unverified causal interpretation
- **What would make us wrong:** A price-only story or weak company transmission

### 10. 2838.TW 聯邦銀 — WAIT

- **WHY:** WHY unresolved: obtain company evidence before taking risk
- **Driver:** UNMAPPED / WHY?
- **Driver state:** UNVERIFIED
- **Company transmission:** Unverified
- **International confirmation:** Unverified — same-driver evidence required
- **Relative:** UNVERIFIED
- **Regime:** UNKNOWN
- **Technical state:** Early strength / support recovery
- **Why price:** Observed 120-session resistance offers at least 2R before costs; support defines <=8% price risk
- **Entry:** 21–21
- **Invalidation:** Exit on loss of 19.95; thesis failure also invalidates
- **Add trigger:** Close above 21.45 with volume >=1.2x prior median; thesis still supported and recheck R/R
- **Main counter-evidence:** Unverified
- **Main risk:** Unverified causal interpretation
- **What would make us wrong:** A price-only story or weak company transmission

### 11. 2023.TW 燁輝 — WAIT

- **WHY:** WHY unresolved: obtain company evidence before taking risk
- **Driver:** UNMAPPED / WHY?
- **Driver state:** UNVERIFIED
- **Company transmission:** Unverified
- **International confirmation:** Unverified — same-driver evidence required
- **Relative:** UNVERIFIED
- **Regime:** UNKNOWN
- **Technical state:** Early strength / support recovery
- **Why price:** Observed 120-session resistance offers at least 2R before costs; support defines <=8% price risk
- **Entry:** 13.85–13.85
- **Invalidation:** Exit on loss of 13.5; thesis failure also invalidates
- **Add trigger:** Close above 14.3 with volume >=1.2x prior median; thesis still supported and recheck R/R
- **Main counter-evidence:** Unverified
- **Main risk:** Unverified causal interpretation
- **What would make us wrong:** A price-only story or weak company transmission

### 12. 2548.TW 華固 — WAIT

- **WHY:** WHY unresolved: obtain company evidence before taking risk
- **Driver:** UNMAPPED / WHY?
- **Driver state:** UNVERIFIED
- **Company transmission:** Unverified
- **International confirmation:** Unverified — same-driver evidence required
- **Relative:** UNVERIFIED
- **Regime:** UNKNOWN
- **Technical state:** Early strength / support recovery
- **Why price:** Observed 120-session resistance offers at least 2R before costs; support defines <=8% price risk
- **Entry:** 94.6–94.9
- **Invalidation:** Exit on loss of 92.5; thesis failure also invalidates
- **Add trigger:** Close above 98.7 with volume >=1.2x prior median; thesis still supported and recheck R/R
- **Main counter-evidence:** Unverified
- **Main risk:** Unverified causal interpretation
- **What would make us wrong:** A price-only story or weak company transmission

### 13. 2449.TW 京元電子 — WAIT

- **WHY:** WHY unresolved: obtain company evidence before taking risk
- **Driver:** UNMAPPED / WHY?
- **Driver state:** UNVERIFIED
- **Company transmission:** Unverified
- **International confirmation:** Unverified — same-driver evidence required
- **Relative:** UNVERIFIED
- **Regime:** UNKNOWN
- **Technical state:** Early strength / support recovery
- **Why price:** Observed 120-session resistance offers at least 2R before costs; support defines <=8% price risk
- **Entry:** 260–263
- **Invalidation:** Exit on loss of 246; thesis failure also invalidates
- **Add trigger:** Close above 287 with volume >=1.2x prior median; thesis still supported and recheck R/R
- **Main counter-evidence:** Unverified
- **Main risk:** Unverified causal interpretation
- **What would make us wrong:** A price-only story or weak company transmission

### 14. 6742.TW 澤米 — WAIT

- **WHY:** WHY unresolved: obtain company evidence before taking risk
- **Driver:** UNMAPPED / WHY?
- **Driver state:** UNVERIFIED
- **Company transmission:** Unverified
- **International confirmation:** Unverified — same-driver evidence required
- **Relative:** UNVERIFIED
- **Regime:** UNKNOWN
- **Technical state:** Early strength / support recovery
- **Why price:** Observed 120-session resistance offers at least 2R before costs; support defines <=8% price risk
- **Entry:** 44.8–45.2
- **Invalidation:** Exit on loss of 42.45; thesis failure also invalidates
- **Add trigger:** Close above 50.15 with volume >=1.2x prior median; thesis still supported and recheck R/R
- **Main counter-evidence:** Unverified
- **Main risk:** Unverified causal interpretation
- **What would make us wrong:** A price-only story or weak company transmission

### 15. 6112.TW 邁達特 — WAIT

- **WHY:** WHY unresolved: obtain company evidence before taking risk
- **Driver:** UNMAPPED / WHY?
- **Driver state:** UNVERIFIED
- **Company transmission:** Unverified
- **International confirmation:** Unverified — same-driver evidence required
- **Relative:** UNVERIFIED
- **Regime:** UNKNOWN
- **Technical state:** Early strength / support recovery
- **Why price:** Observed 120-session resistance offers at least 2R before costs; support defines <=8% price risk
- **Entry:** 45.85–46
- **Invalidation:** Exit on loss of 43.85; thesis failure also invalidates
- **Add trigger:** Close above 47.65 with volume >=1.2x prior median; thesis still supported and recheck R/R
- **Main counter-evidence:** Unverified
- **Main risk:** Unverified causal interpretation
- **What would make us wrong:** A price-only story or weak company transmission

### 16. 6894.TWO 衛司特 — WAIT

- **WHY:** WHY unresolved: obtain company evidence before taking risk
- **Driver:** UNMAPPED / WHY?
- **Driver state:** UNVERIFIED
- **Company transmission:** Unverified
- **International confirmation:** Unverified — same-driver evidence required
- **Relative:** UNVERIFIED
- **Regime:** UNKNOWN
- **Technical state:** Early strength / support recovery
- **Why price:** Observed 120-session resistance offers at least 2R before costs; support defines <=8% price risk
- **Entry:** 353.5–356
- **Invalidation:** Exit on loss of 328; thesis failure also invalidates
- **Add trigger:** Close above 377 with volume >=1.2x prior median; thesis still supported and recheck R/R
- **Main counter-evidence:** Unverified
- **Main risk:** Unverified causal interpretation
- **What would make us wrong:** A price-only story or weak company transmission

### 17. 6179.TWO 亞通 — WAIT

- **WHY:** WHY unresolved: obtain company evidence before taking risk
- **Driver:** Unmapped opportunity — research WHY
- **Driver state:** UNVERIFIED
- **Company transmission:** Unverified
- **International confirmation:** Unverified — same-driver evidence required
- **Relative:** UNVERIFIED
- **Regime:** UNKNOWN
- **Technical state:** Early strength / support recovery
- **Why price:** Wait for a supported pullback with >=2R to observed resistance and <=8% stop distance
- **Entry:** 35.3–35.75
- **Invalidation:** Exit on loss of 27.35; thesis failure also invalidates
- **Add trigger:** Close above 40 with volume >=1.2x prior median; thesis still supported and recheck R/R
- **Main counter-evidence:** Unverified
- **Main risk:** Unverified causal interpretation
- **What would make us wrong:** A price-only story or weak company transmission

### 18. 2409.TW 友達 — WAIT

- **WHY:** WHY unresolved: obtain company evidence before taking risk
- **Driver:** Unmapped opportunity — research WHY
- **Driver state:** UNVERIFIED
- **Company transmission:** Unverified
- **International confirmation:** Unverified — same-driver evidence required
- **Relative:** UNVERIFIED
- **Regime:** UNKNOWN
- **Technical state:** Early strength / support recovery
- **Why price:** Wait for a supported pullback with >=2R to observed resistance and <=8% stop distance
- **Entry:** 29.85–30.2
- **Invalidation:** Exit on loss of 24.8; thesis failure also invalidates
- **Add trigger:** Close above 32.6 with volume >=1.2x prior median; thesis still supported and recheck R/R
- **Main counter-evidence:** Unverified
- **Main risk:** Unverified causal interpretation
- **What would make us wrong:** A price-only story or weak company transmission

### 19. 6739.TWO 竹陞科技 — WAIT

- **WHY:** WHY unresolved: obtain company evidence before taking risk
- **Driver:** Unmapped opportunity — research WHY
- **Driver state:** UNVERIFIED
- **Company transmission:** Unverified
- **International confirmation:** Unverified — same-driver evidence required
- **Relative:** UNVERIFIED
- **Regime:** UNKNOWN
- **Technical state:** Early strength / support recovery
- **Why price:** Wait for a supported pullback with >=2R to observed resistance and <=8% stop distance
- **Entry:** 1255–1270
- **Invalidation:** Exit on loss of 1055; thesis failure also invalidates
- **Add trigger:** Close above 1340 with volume >=1.2x prior median; thesis still supported and recheck R/R
- **Main counter-evidence:** Unverified
- **Main risk:** Unverified causal interpretation
- **What would make us wrong:** A price-only story or weak company transmission

### 20. 2069.TW 運錩 — WAIT

- **WHY:** WHY unresolved: obtain company evidence before taking risk
- **Driver:** Unmapped opportunity — research WHY
- **Driver state:** UNVERIFIED
- **Company transmission:** Unverified
- **International confirmation:** Unverified — same-driver evidence required
- **Relative:** UNVERIFIED
- **Regime:** UNKNOWN
- **Technical state:** Confirmed breakout
- **Why price:** Wait for a supported pullback with >=2R to observed resistance and <=8% stop distance
- **Entry:** 24.45–24.6
- **Invalidation:** Exit on loss of 20.15; thesis failure also invalidates
- **Add trigger:** Close above 24.75 with volume >=1.2x prior median; thesis still supported and recheck R/R
- **Main counter-evidence:** Unverified
- **Main risk:** Unverified causal interpretation
- **What would make us wrong:** A price-only story or weak company transmission

### 21. 6426.TW 統新 — WAIT

- **WHY:** WHY unresolved: obtain company evidence before taking risk
- **Driver:** Unmapped opportunity — research WHY
- **Driver state:** UNVERIFIED
- **Company transmission:** Unverified
- **International confirmation:** Unverified — same-driver evidence required
- **Relative:** UNVERIFIED
- **Regime:** UNKNOWN
- **Technical state:** Early strength / support recovery
- **Why price:** Wait for a supported pullback with >=2R to observed resistance and <=8% stop distance
- **Entry:** 296.5–302
- **Invalidation:** Exit on loss of 243; thesis failure also invalidates
- **Add trigger:** Close above 334 with volume >=1.2x prior median; thesis still supported and recheck R/R
- **Main counter-evidence:** Unverified
- **Main risk:** Unverified causal interpretation
- **What would make us wrong:** A price-only story or weak company transmission

### 22. 5340.TWO 建榮 — WAIT

- **WHY:** WHY unresolved: obtain company evidence before taking risk
- **Driver:** Unmapped opportunity — research WHY
- **Driver state:** UNVERIFIED
- **Company transmission:** Unverified
- **International confirmation:** Unverified — same-driver evidence required
- **Relative:** UNVERIFIED
- **Regime:** UNKNOWN
- **Technical state:** Early strength / support recovery
- **Why price:** Wait for a supported pullback with >=2R to observed resistance and <=8% stop distance
- **Entry:** 84.9–85.9
- **Invalidation:** Exit on loss of 75.7; thesis failure also invalidates
- **Add trigger:** Close above 89.9 with volume >=1.2x prior median; thesis still supported and recheck R/R
- **Main counter-evidence:** Unverified
- **Main risk:** Unverified causal interpretation
- **What would make us wrong:** A price-only story or weak company transmission

### 23. 3623.TWO 富晶通 — WAIT

- **WHY:** WHY unresolved: obtain company evidence before taking risk
- **Driver:** Unmapped opportunity — research WHY
- **Driver state:** UNVERIFIED
- **Company transmission:** Unverified
- **International confirmation:** Unverified — same-driver evidence required
- **Relative:** UNVERIFIED
- **Regime:** UNKNOWN
- **Technical state:** Confirmed breakout
- **Why price:** Wait for a supported pullback with >=2R to observed resistance and <=8% stop distance
- **Entry:** 26.95–27.1
- **Invalidation:** Exit on loss of 21.7; thesis failure also invalidates
- **Add trigger:** Close above 27.35 with volume >=1.2x prior median; thesis still supported and recheck R/R
- **Main counter-evidence:** Unverified
- **Main risk:** Unverified causal interpretation
- **What would make us wrong:** A price-only story or weak company transmission

### 24. 6505.TW 台塑化 — WAIT

- **WHY:** WHY unresolved: obtain company evidence before taking risk
- **Driver:** Unmapped opportunity — research WHY
- **Driver state:** UNVERIFIED
- **Company transmission:** Unverified
- **International confirmation:** Unverified — same-driver evidence required
- **Relative:** UNVERIFIED
- **Regime:** UNKNOWN
- **Technical state:** Early strength / support recovery
- **Why price:** Wait for a supported pullback with >=2R to observed resistance and <=8% stop distance
- **Entry:** 82.7–83.5
- **Invalidation:** Exit on loss of 70.1; thesis failure also invalidates
- **Add trigger:** Close above 88.5 with volume >=1.2x prior median; thesis still supported and recheck R/R
- **Main counter-evidence:** Unverified
- **Main risk:** Unverified causal interpretation
- **What would make us wrong:** A price-only story or weak company transmission

### 25. 6885.TW 全福生技 — WAIT

- **WHY:** WHY unresolved: obtain company evidence before taking risk
- **Driver:** Unmapped opportunity — research WHY
- **Driver state:** UNVERIFIED
- **Company transmission:** Unverified
- **International confirmation:** Unverified — same-driver evidence required
- **Relative:** UNVERIFIED
- **Regime:** UNKNOWN
- **Technical state:** Early strength / support recovery
- **Why price:** Wait for a supported pullback with >=2R to observed resistance and <=8% stop distance
- **Entry:** 26.8–27.1
- **Invalidation:** Exit on loss of 21.8; thesis failure also invalidates
- **Add trigger:** Close above 27.5 with volume >=1.2x prior median; thesis still supported and recheck R/R
- **Main counter-evidence:** Unverified
- **Main risk:** Unverified causal interpretation
- **What would make us wrong:** A price-only story or weak company transmission

### 26. 7717.TWO 萊德光電-KY — WAIT

- **WHY:** WHY unresolved: obtain company evidence before taking risk
- **Driver:** Unmapped opportunity — research WHY
- **Driver state:** UNVERIFIED
- **Company transmission:** Unverified
- **International confirmation:** Unverified — same-driver evidence required
- **Relative:** UNVERIFIED
- **Regime:** UNKNOWN
- **Technical state:** Early strength / support recovery
- **Why price:** Wait for a supported pullback with >=2R to observed resistance and <=8% stop distance
- **Entry:** 509–517
- **Invalidation:** Exit on loss of 400.5; thesis failure also invalidates
- **Add trigger:** Close above 528 with volume >=1.2x prior median; thesis still supported and recheck R/R
- **Main counter-evidence:** Unverified
- **Main risk:** Unverified causal interpretation
- **What would make us wrong:** A price-only story or weak company transmission

### 27. 2609.TW 陽明 — WAIT

- **WHY:** WHY unresolved: obtain company evidence before taking risk
- **Driver:** Container freight / liner shipping cycle
- **Driver state:** UNVERIFIED
- **Company transmission:** Unverified
- **International confirmation:** Unverified — same-driver evidence required
- **Relative:** UNVERIFIED
- **Regime:** UNKNOWN
- **Technical state:** Early strength / support recovery
- **Why price:** Wait for a supported pullback with >=2R to observed resistance and <=8% stop distance
- **Entry:** 58.8–59.4
- **Invalidation:** Exit on loss of 55.1; thesis failure also invalidates
- **Add trigger:** Close above 68 with volume >=1.2x prior median; thesis still supported and recheck R/R
- **Main counter-evidence:** Unverified
- **Main risk:** Unverified causal interpretation
- **What would make us wrong:** A price-only story or weak company transmission

### 28. 2617.TW 台航 — WAIT

- **WHY:** WHY unresolved: obtain company evidence before taking risk
- **Driver:** Dry-bulk freight / commodity shipping cycle
- **Driver state:** UNVERIFIED
- **Company transmission:** Unverified
- **International confirmation:** Unverified — same-driver evidence required
- **Relative:** UNVERIFIED
- **Regime:** UNKNOWN
- **Technical state:** Early strength / support recovery
- **Why price:** Wait for a supported pullback with >=2R to observed resistance and <=8% stop distance
- **Entry:** 33.4–33.65
- **Invalidation:** Exit on loss of 30; thesis failure also invalidates
- **Add trigger:** Close above 34.4 with volume >=1.2x prior median; thesis still supported and recheck R/R
- **Main counter-evidence:** Unverified
- **Main risk:** Unverified causal interpretation
- **What would make us wrong:** A price-only story or weak company transmission

### 29. 3006.TW 晶豪科 — WAIT

- **WHY:** WHY unresolved: obtain company evidence before taking risk
- **Driver:** Memory IC design demand / ASP cycle
- **Driver state:** UNVERIFIED
- **Company transmission:** Unverified
- **International confirmation:** Unverified — same-driver evidence required
- **Relative:** UNVERIFIED
- **Regime:** UNKNOWN
- **Technical state:** Early strength / support recovery
- **Why price:** Wait for a supported pullback with >=2R to observed resistance and <=8% stop distance
- **Entry:** 285.5–290.5
- **Invalidation:** Exit on loss of 258; thesis failure also invalidates
- **Add trigger:** Close above 324.5 with volume >=1.2x prior median; thesis still supported and recheck R/R
- **Main counter-evidence:** Unverified
- **Main risk:** Unverified causal interpretation
- **What would make us wrong:** A price-only story or weak company transmission

### 30. 2603.TW 長榮 — WAIT

- **WHY:** WHY unresolved: obtain company evidence before taking risk
- **Driver:** Container freight / liner shipping cycle
- **Driver state:** UNVERIFIED
- **Company transmission:** Unverified
- **International confirmation:** Unverified — same-driver evidence required
- **Relative:** UNVERIFIED
- **Regime:** UNKNOWN
- **Technical state:** No early strength
- **Why price:** Wait for a supported pullback with >=2R to observed resistance and <=8% stop distance
- **Entry:** 233–234.5
- **Invalidation:** Exit on loss of 225; thesis failure also invalidates
- **Add trigger:** Close above 258.5 with volume >=1.2x prior median; thesis still supported and recheck R/R
- **Main counter-evidence:** Unverified
- **Main risk:** Unverified causal interpretation
- **What would make us wrong:** A price-only story or weak company transmission

### 31. 2606.TW 裕民 — WAIT

- **WHY:** WHY unresolved: obtain company evidence before taking risk
- **Driver:** Dry-bulk freight / commodity shipping cycle
- **Driver state:** UNVERIFIED
- **Company transmission:** Unverified
- **International confirmation:** Unverified — same-driver evidence required
- **Relative:** UNVERIFIED
- **Regime:** UNKNOWN
- **Technical state:** No early strength
- **Why price:** Wait for a supported pullback with >=2R to observed resistance and <=8% stop distance
- **Entry:** 75.5–76.5
- **Invalidation:** Exit on loss of 69.8; thesis failure also invalidates
- **Add trigger:** Close above 82.8 with volume >=1.2x prior median; thesis still supported and recheck R/R
- **Main counter-evidence:** Unverified
- **Main risk:** Unverified causal interpretation
- **What would make us wrong:** A price-only story or weak company transmission

### 32. 2637.TW 慧洋-KY — WAIT

- **WHY:** WHY unresolved: obtain company evidence before taking risk
- **Driver:** Dry-bulk freight / commodity shipping cycle
- **Driver state:** UNVERIFIED
- **Company transmission:** Unverified
- **International confirmation:** Unverified — same-driver evidence required
- **Relative:** UNVERIFIED
- **Regime:** UNKNOWN
- **Technical state:** Early strength / support recovery
- **Why price:** Wait for a supported pullback with >=2R to observed resistance and <=8% stop distance
- **Entry:** 100–101
- **Invalidation:** Exit on loss of 89.3; thesis failure also invalidates
- **Add trigger:** Close above 109.5 with volume >=1.2x prior median; thesis still supported and recheck R/R
- **Main counter-evidence:** Unverified
- **Main risk:** Unverified causal interpretation
- **What would make us wrong:** A price-only story or weak company transmission

### 33. 8996.TW 高力 — WAIT

- **WHY:** WHY unresolved: obtain company evidence before taking risk
- **Driver:** AI server thermal density / liquid cooling
- **Driver state:** UNVERIFIED
- **Company transmission:** Unverified
- **International confirmation:** Unverified — same-driver evidence required
- **Relative:** UNVERIFIED
- **Regime:** UNKNOWN
- **Technical state:** Early strength / support recovery
- **Why price:** Wait for a supported pullback with >=2R to observed resistance and <=8% stop distance
- **Entry:** 1305–1330
- **Invalidation:** Exit on loss of 1110; thesis failure also invalidates
- **Add trigger:** Close above 1440 with volume >=1.2x prior median; thesis still supported and recheck R/R
- **Main counter-evidence:** Unverified
- **Main risk:** Unverified causal interpretation
- **What would make us wrong:** A price-only story or weak company transmission

### 34. 2303.TW 聯電 — WAIT

- **WHY:** WHY unresolved: obtain company evidence before taking risk
- **Driver:** Mature-node foundry utilization / pricing
- **Driver state:** UNVERIFIED
- **Company transmission:** Unverified
- **International confirmation:** Unverified — same-driver evidence required
- **Relative:** UNVERIFIED
- **Regime:** UNKNOWN
- **Technical state:** No early strength
- **Why price:** Wait for a supported pullback with >=2R to observed resistance and <=8% stop distance
- **Entry:** 139–140.5
- **Invalidation:** Exit on loss of 118; thesis failure also invalidates
- **Add trigger:** Close above 147.5 with volume >=1.2x prior median; thesis still supported and recheck R/R
- **Main counter-evidence:** Unverified
- **Main risk:** Unverified causal interpretation
- **What would make us wrong:** A price-only story or weak company transmission

### 35. 2330.TW 台積電 — WAIT

- **WHY:** WHY unresolved: obtain company evidence before taking risk
- **Driver:** Leading-edge foundry AI/HPC node demand
- **Driver state:** UNVERIFIED
- **Company transmission:** Unverified
- **International confirmation:** Unverified — same-driver evidence required
- **Relative:** UNVERIFIED
- **Regime:** UNKNOWN
- **Technical state:** No early strength
- **Why price:** Wait for a supported pullback with >=2R to observed resistance and <=8% stop distance
- **Entry:** 2405–2410
- **Invalidation:** Exit on loss of 2365; thesis failure also invalidates
- **Add trigger:** Close above 2515 with volume >=1.2x prior median; thesis still supported and recheck R/R
- **Main counter-evidence:** Unverified
- **Main risk:** Unverified causal interpretation
- **What would make us wrong:** A price-only story or weak company transmission

### 36. 2886.TW 兆豐金 — WAIT

- **WHY:** WHY unresolved: obtain company evidence before taking risk
- **Driver:** Financials rate / credit / margin cycle
- **Driver state:** UNVERIFIED
- **Company transmission:** Unverified
- **International confirmation:** Unverified — same-driver evidence required
- **Relative:** UNVERIFIED
- **Regime:** UNKNOWN
- **Technical state:** Confirmed breakout
- **Why price:** Wait for a supported pullback with >=2R to observed resistance and <=8% stop distance
- **Entry:** 51.5–51.7
- **Invalidation:** Exit on loss of 46.25; thesis failure also invalidates
- **Add trigger:** Close above 51.2 with volume >=1.2x prior median; thesis still supported and recheck R/R
- **Main counter-evidence:** Unverified
- **Main risk:** Unverified causal interpretation
- **What would make us wrong:** A price-only story or weak company transmission

### 37. 6706.TW 惠特 — WAIT

- **WHY:** WHY unresolved: obtain company evidence before taking risk
- **Driver:** UNMAPPED / WHY?
- **Driver state:** UNVERIFIED
- **Company transmission:** Unverified
- **International confirmation:** Unverified — same-driver evidence required
- **Relative:** UNVERIFIED
- **Regime:** UNKNOWN
- **Technical state:** Early strength / support recovery
- **Why price:** Wait for a supported pullback with >=2R to observed resistance and <=8% stop distance
- **Entry:** 136–137.5
- **Invalidation:** Exit on loss of 117; thesis failure also invalidates
- **Add trigger:** Close above 140 with volume >=1.2x prior median; thesis still supported and recheck R/R
- **Main counter-evidence:** Unverified
- **Main risk:** Unverified causal interpretation
- **What would make us wrong:** A price-only story or weak company transmission

### 38. 2412.TW 中華電 — WAIT

- **WHY:** WHY unresolved: obtain company evidence before taking risk
- **Driver:** UNMAPPED / WHY?
- **Driver state:** UNVERIFIED
- **Company transmission:** Unverified
- **International confirmation:** Unverified — same-driver evidence required
- **Relative:** UNVERIFIED
- **Regime:** UNKNOWN
- **Technical state:** Confirmed breakout
- **Why price:** Wait for a supported pullback with >=2R to observed resistance and <=8% stop distance
- **Entry:** 140.5–140.5
- **Invalidation:** Exit on loss of 134.5; thesis failure also invalidates
- **Add trigger:** Close above 140 with volume >=1.2x prior median; thesis still supported and recheck R/R
- **Main counter-evidence:** Unverified
- **Main risk:** Unverified causal interpretation
- **What would make us wrong:** A price-only story or weak company transmission

### 39. 7825.TWO 和亞智慧 — WAIT

- **WHY:** WHY unresolved: obtain company evidence before taking risk
- **Driver:** UNMAPPED / WHY?
- **Driver state:** UNVERIFIED
- **Company transmission:** Unverified
- **International confirmation:** Unverified — same-driver evidence required
- **Relative:** UNVERIFIED
- **Regime:** UNKNOWN
- **Technical state:** Early strength / support recovery
- **Why price:** Wait for a supported pullback with >=2R to observed resistance and <=8% stop distance
- **Entry:** 105–107.5
- **Invalidation:** Exit on loss of 85.4; thesis failure also invalidates
- **Add trigger:** Close above 122.5 with volume >=1.2x prior median; thesis still supported and recheck R/R
- **Main counter-evidence:** Unverified
- **Main risk:** Unverified causal interpretation
- **What would make us wrong:** A price-only story or weak company transmission

### 40. 2413.TW 環科 — WAIT

- **WHY:** WHY unresolved: obtain company evidence before taking risk
- **Driver:** UNMAPPED / WHY?
- **Driver state:** UNVERIFIED
- **Company transmission:** Unverified
- **International confirmation:** Unverified — same-driver evidence required
- **Relative:** UNVERIFIED
- **Regime:** UNKNOWN
- **Technical state:** Early strength / support recovery
- **Why price:** Wait for a supported pullback with >=2R to observed resistance and <=8% stop distance
- **Entry:** 49.7–50.2
- **Invalidation:** Exit on loss of 40.95; thesis failure also invalidates
- **Add trigger:** Close above 53.8 with volume >=1.2x prior median; thesis still supported and recheck R/R
- **Main counter-evidence:** Unverified
- **Main risk:** Unverified causal interpretation
- **What would make us wrong:** A price-only story or weak company transmission

### 41. 6416.TW 瑞祺電通 — WAIT

- **WHY:** WHY unresolved: obtain company evidence before taking risk
- **Driver:** UNMAPPED / WHY?
- **Driver state:** UNVERIFIED
- **Company transmission:** Unverified
- **International confirmation:** Unverified — same-driver evidence required
- **Relative:** UNVERIFIED
- **Regime:** UNKNOWN
- **Technical state:** Confirmed breakout
- **Why price:** Wait for a supported pullback with >=2R to observed resistance and <=8% stop distance
- **Entry:** 115–115.5
- **Invalidation:** Exit on loss of 94.5; thesis failure also invalidates
- **Add trigger:** Close above 116 with volume >=1.2x prior median; thesis still supported and recheck R/R
- **Main counter-evidence:** Unverified
- **Main risk:** Unverified causal interpretation
- **What would make us wrong:** A price-only story or weak company transmission

### 42. 7799.TW 禾榮科 — WAIT

- **WHY:** WHY unresolved: obtain company evidence before taking risk
- **Driver:** UNMAPPED / WHY?
- **Driver state:** UNVERIFIED
- **Company transmission:** Unverified
- **International confirmation:** Unverified — same-driver evidence required
- **Relative:** UNVERIFIED
- **Regime:** UNKNOWN
- **Technical state:** Early strength / support recovery
- **Why price:** Wait for a supported pullback with >=2R to observed resistance and <=8% stop distance
- **Entry:** 410–414.5
- **Invalidation:** Exit on loss of 347; thesis failure also invalidates
- **Add trigger:** Close above 430 with volume >=1.2x prior median; thesis still supported and recheck R/R
- **Main counter-evidence:** Unverified
- **Main risk:** Unverified causal interpretation
- **What would make us wrong:** A price-only story or weak company transmission

### 43. 6770.TW 力積電 — WAIT

- **WHY:** WHY unresolved: obtain company evidence before taking risk
- **Driver:** Mature-node foundry utilization / pricing
- **Driver state:** UNVERIFIED
- **Company transmission:** Unverified
- **International confirmation:** Unverified — same-driver evidence required
- **Relative:** UNVERIFIED
- **Regime:** UNKNOWN
- **Technical state:** No early strength
- **Why price:** Wait for a supported pullback with >=2R to observed resistance and <=8% stop distance
- **Entry:** 69.8–70.6
- **Invalidation:** Exit on loss of 66; thesis failure also invalidates
- **Add trigger:** Close above 82.8 with volume >=1.2x prior median; thesis still supported and recheck R/R
- **Main counter-evidence:** Unverified
- **Main risk:** Unverified causal interpretation
- **What would make us wrong:** A price-only story or weak company transmission

### 44. 2466.TW 冠西電 — WAIT

- **WHY:** WHY unresolved: obtain company evidence before taking risk
- **Driver:** UNMAPPED / WHY?
- **Driver state:** UNVERIFIED
- **Company transmission:** Unverified
- **International confirmation:** Unverified — same-driver evidence required
- **Relative:** UNVERIFIED
- **Regime:** UNKNOWN
- **Technical state:** Early strength / support recovery
- **Why price:** Wait for a supported pullback with >=2R to observed resistance and <=8% stop distance
- **Entry:** 105–106
- **Invalidation:** Exit on loss of 93.2; thesis failure also invalidates
- **Add trigger:** Close above 112.5 with volume >=1.2x prior median; thesis still supported and recheck R/R
- **Main counter-evidence:** Unverified
- **Main risk:** Unverified causal interpretation
- **What would make us wrong:** A price-only story or weak company transmission

### 45. 3443.TW 創意 — WAIT

- **WHY:** WHY unresolved: obtain company evidence before taking risk
- **Driver:** UNMAPPED / WHY?
- **Driver state:** UNVERIFIED
- **Company transmission:** Unverified
- **International confirmation:** Unverified — same-driver evidence required
- **Relative:** UNVERIFIED
- **Regime:** UNKNOWN
- **Technical state:** No early strength
- **Why price:** Wait for a supported pullback with >=2R to observed resistance and <=8% stop distance
- **Entry:** 6030–6120
- **Invalidation:** Exit on loss of 5585; thesis failure also invalidates
- **Add trigger:** Close above 6585 with volume >=1.2x prior median; thesis still supported and recheck R/R
- **Main counter-evidence:** Unverified
- **Main risk:** Unverified causal interpretation
- **What would make us wrong:** A price-only story or weak company transmission

### 46. 2254.TW 巨鎧精密-創 — WAIT

- **WHY:** WHY unresolved: obtain company evidence before taking risk
- **Driver:** UNMAPPED / WHY?
- **Driver state:** UNVERIFIED
- **Company transmission:** Unverified
- **International confirmation:** Unverified — same-driver evidence required
- **Relative:** UNVERIFIED
- **Regime:** UNKNOWN
- **Technical state:** Early strength / support recovery
- **Why price:** Wait for a supported pullback with >=2R to observed resistance and <=8% stop distance
- **Entry:** 65.4–65.8
- **Invalidation:** Exit on loss of 59.7; thesis failure also invalidates
- **Add trigger:** Close above 67.8 with volume >=1.2x prior median; thesis still supported and recheck R/R
- **Main counter-evidence:** Unverified
- **Main risk:** Unverified causal interpretation
- **What would make us wrong:** A price-only story or weak company transmission

### 47. 2881.TW 富邦金 — WAIT

- **WHY:** WHY unresolved: obtain company evidence before taking risk
- **Driver:** Financials rate / credit / margin cycle
- **Driver state:** UNVERIFIED
- **Company transmission:** Unverified
- **International confirmation:** Unverified — same-driver evidence required
- **Relative:** UNVERIFIED
- **Regime:** UNKNOWN
- **Technical state:** No early strength
- **Why price:** Wait for a supported pullback with >=2R to observed resistance and <=8% stop distance
- **Entry:** 147.5–148.5
- **Invalidation:** Exit on loss of 139; thesis failure also invalidates
- **Add trigger:** Close above 157 with volume >=1.2x prior median; thesis still supported and recheck R/R
- **Main counter-evidence:** Unverified
- **Main risk:** Unverified causal interpretation
- **What would make us wrong:** A price-only story or weak company transmission

### 48. 2882.TW 國泰金 — WAIT

- **WHY:** WHY unresolved: obtain company evidence before taking risk
- **Driver:** Financials rate / credit / margin cycle
- **Driver state:** UNVERIFIED
- **Company transmission:** Unverified
- **International confirmation:** Unverified — same-driver evidence required
- **Relative:** UNVERIFIED
- **Regime:** UNKNOWN
- **Technical state:** No early strength
- **Why price:** Wait for a supported pullback with >=2R to observed resistance and <=8% stop distance
- **Entry:** 110–110.5
- **Invalidation:** Exit on loss of 102; thesis failure also invalidates
- **Add trigger:** Close above 118 with volume >=1.2x prior median; thesis still supported and recheck R/R
- **Main counter-evidence:** Unverified
- **Main risk:** Unverified causal interpretation
- **What would make us wrong:** A price-only story or weak company transmission

### 49. 2801.TW 彰銀 — WAIT

- **WHY:** WHY unresolved: obtain company evidence before taking risk
- **Driver:** UNMAPPED / WHY?
- **Driver state:** UNVERIFIED
- **Company transmission:** Unverified
- **International confirmation:** Unverified — same-driver evidence required
- **Relative:** UNVERIFIED
- **Regime:** UNKNOWN
- **Technical state:** Confirmed breakout
- **Why price:** Wait for a supported pullback with >=2R to observed resistance and <=8% stop distance
- **Entry:** 27.75–27.85
- **Invalidation:** Exit on loss of 24.45; thesis failure also invalidates
- **Add trigger:** Close above 27.75 with volume >=1.2x prior median; thesis still supported and recheck R/R
- **Main counter-evidence:** Unverified
- **Main risk:** Unverified causal interpretation
- **What would make us wrong:** A price-only story or weak company transmission

### 50. 4303.TWO 信立 — WAIT

- **WHY:** WHY unresolved: obtain company evidence before taking risk
- **Driver:** UNMAPPED / WHY?
- **Driver state:** UNVERIFIED
- **Company transmission:** Unverified
- **International confirmation:** Unverified — same-driver evidence required
- **Relative:** UNVERIFIED
- **Regime:** UNKNOWN
- **Technical state:** No early strength
- **Why price:** Wait for a supported pullback with >=2R to observed resistance and <=8% stop distance
- **Entry:** 36.35–36.8
- **Invalidation:** Exit on loss of 33.7; thesis failure also invalidates
- **Add trigger:** Close above 45.45 with volume >=1.2x prior median; thesis still supported and recheck R/R
- **Main counter-evidence:** Unverified
- **Main risk:** Unverified causal interpretation
- **What would make us wrong:** A price-only story or weak company transmission

### 51. 6870.TWO 騰雲 — WAIT

- **WHY:** WHY unresolved: obtain company evidence before taking risk
- **Driver:** UNMAPPED / WHY?
- **Driver state:** UNVERIFIED
- **Company transmission:** Unverified
- **International confirmation:** Unverified — same-driver evidence required
- **Relative:** UNVERIFIED
- **Regime:** UNKNOWN
- **Technical state:** Early strength / support recovery
- **Why price:** Wait for a supported pullback with >=2R to observed resistance and <=8% stop distance
- **Entry:** 303–306.5
- **Invalidation:** Exit on loss of 245; thesis failure also invalidates
- **Add trigger:** Close above 319 with volume >=1.2x prior median; thesis still supported and recheck R/R
- **Main counter-evidence:** Unverified
- **Main risk:** Unverified causal interpretation
- **What would make us wrong:** A price-only story or weak company transmission

### 52. 2243.TW 宏旭-KY — WAIT

- **WHY:** WHY unresolved: obtain company evidence before taking risk
- **Driver:** UNMAPPED / WHY?
- **Driver state:** UNVERIFIED
- **Company transmission:** Unverified
- **International confirmation:** Unverified — same-driver evidence required
- **Relative:** UNVERIFIED
- **Regime:** UNKNOWN
- **Technical state:** Early strength / support recovery
- **Why price:** Wait for a supported pullback with >=2R to observed resistance and <=8% stop distance
- **Entry:** 49.4–50
- **Invalidation:** Exit on loss of 40.3; thesis failure also invalidates
- **Add trigger:** Close above 51.5 with volume >=1.2x prior median; thesis still supported and recheck R/R
- **Main counter-evidence:** Unverified
- **Main risk:** Unverified causal interpretation
- **What would make us wrong:** A price-only story or weak company transmission

### 53. 2884.TW 玉山金 — WAIT

- **WHY:** WHY unresolved: obtain company evidence before taking risk
- **Driver:** UNMAPPED / WHY?
- **Driver state:** UNVERIFIED
- **Company transmission:** Unverified
- **International confirmation:** Unverified — same-driver evidence required
- **Relative:** UNVERIFIED
- **Regime:** UNKNOWN
- **Technical state:** Early strength / support recovery
- **Why price:** Wait for a supported pullback with >=2R to observed resistance and <=8% stop distance
- **Entry:** 46–46.25
- **Invalidation:** Exit on loss of 37.75; thesis failure also invalidates
- **Add trigger:** Close above 46.4 with volume >=1.2x prior median; thesis still supported and recheck R/R
- **Main counter-evidence:** Unverified
- **Main risk:** Unverified causal interpretation
- **What would make us wrong:** A price-only story or weak company transmission

### 54. 1815.TWO 富喬 — WAIT

- **WHY:** WHY unresolved: obtain company evidence before taking risk
- **Driver:** UNMAPPED / WHY?
- **Driver state:** UNVERIFIED
- **Company transmission:** Unverified
- **International confirmation:** Unverified — same-driver evidence required
- **Relative:** UNVERIFIED
- **Regime:** UNKNOWN
- **Technical state:** Early strength / support recovery
- **Why price:** Wait for a supported pullback with >=2R to observed resistance and <=8% stop distance
- **Entry:** 125–127.5
- **Invalidation:** Exit on loss of 103; thesis failure also invalidates
- **Add trigger:** Close above 138 with volume >=1.2x prior median; thesis still supported and recheck R/R
- **Main counter-evidence:** Unverified
- **Main risk:** Unverified causal interpretation
- **What would make us wrong:** A price-only story or weak company transmission

### 55. 3532.TW 台勝科 — WAIT

- **WHY:** WHY unresolved: obtain company evidence before taking risk
- **Driver:** UNMAPPED / WHY?
- **Driver state:** UNVERIFIED
- **Company transmission:** Unverified
- **International confirmation:** Unverified — same-driver evidence required
- **Relative:** UNVERIFIED
- **Regime:** UNKNOWN
- **Technical state:** Early strength / support recovery
- **Why price:** Wait for a supported pullback with >=2R to observed resistance and <=8% stop distance
- **Entry:** 416.5–423
- **Invalidation:** Exit on loss of 360.5; thesis failure also invalidates
- **Add trigger:** Close above 445.5 with volume >=1.2x prior median; thesis still supported and recheck R/R
- **Main counter-evidence:** Unverified
- **Main risk:** Unverified causal interpretation
- **What would make us wrong:** A price-only story or weak company transmission

### 56. 4714.TWO 永捷 — WAIT

- **WHY:** WHY unresolved: obtain company evidence before taking risk
- **Driver:** UNMAPPED / WHY?
- **Driver state:** UNVERIFIED
- **Company transmission:** Unverified
- **International confirmation:** Unverified — same-driver evidence required
- **Relative:** UNVERIFIED
- **Regime:** UNKNOWN
- **Technical state:** Early strength / support recovery
- **Why price:** Wait for a supported pullback with >=2R to observed resistance and <=8% stop distance
- **Entry:** 11.6–11.65
- **Invalidation:** Exit on loss of 10.35; thesis failure also invalidates
- **Add trigger:** Close above 12.55 with volume >=1.2x prior median; thesis still supported and recheck R/R
- **Main counter-evidence:** Unverified
- **Main risk:** Unverified causal interpretation
- **What would make us wrong:** A price-only story or weak company transmission

### 57. 1528.TW 恩德 — WAIT

- **WHY:** WHY unresolved: obtain company evidence before taking risk
- **Driver:** UNMAPPED / WHY?
- **Driver state:** UNVERIFIED
- **Company transmission:** Unverified
- **International confirmation:** Unverified — same-driver evidence required
- **Relative:** UNVERIFIED
- **Regime:** UNKNOWN
- **Technical state:** Early strength / support recovery
- **Why price:** Wait for a supported pullback with >=2R to observed resistance and <=8% stop distance
- **Entry:** 21.05–21.2
- **Invalidation:** Exit on loss of 18.5; thesis failure also invalidates
- **Add trigger:** Close above 22 with volume >=1.2x prior median; thesis still supported and recheck R/R
- **Main counter-evidence:** Unverified
- **Main risk:** Unverified causal interpretation
- **What would make us wrong:** A price-only story or weak company transmission

### 58. 1709.TW 和益 — WAIT

- **WHY:** WHY unresolved: obtain company evidence before taking risk
- **Driver:** UNMAPPED / WHY?
- **Driver state:** UNVERIFIED
- **Company transmission:** Unverified
- **International confirmation:** Unverified — same-driver evidence required
- **Relative:** UNVERIFIED
- **Regime:** UNKNOWN
- **Technical state:** Early strength / support recovery
- **Why price:** Wait for a supported pullback with >=2R to observed resistance and <=8% stop distance
- **Entry:** 35.6–36.2
- **Invalidation:** Exit on loss of 32.15; thesis failure also invalidates
- **Add trigger:** Close above 38.35 with volume >=1.2x prior median; thesis still supported and recheck R/R
- **Main counter-evidence:** Unverified
- **Main risk:** Unverified causal interpretation
- **What would make us wrong:** A price-only story or weak company transmission

### 59. 6133.TW 金橋 — WAIT

- **WHY:** WHY unresolved: obtain company evidence before taking risk
- **Driver:** UNMAPPED / WHY?
- **Driver state:** UNVERIFIED
- **Company transmission:** Unverified
- **International confirmation:** Unverified — same-driver evidence required
- **Relative:** UNVERIFIED
- **Regime:** UNKNOWN
- **Technical state:** No early strength
- **Why price:** Wait for a supported pullback with >=2R to observed resistance and <=8% stop distance
- **Entry:** 23.35–23.5
- **Invalidation:** Exit on loss of 21.75; thesis failure also invalidates
- **Add trigger:** Close above 25.45 with volume >=1.2x prior median; thesis still supported and recheck R/R
- **Main counter-evidence:** Unverified
- **Main risk:** Unverified causal interpretation
- **What would make us wrong:** A price-only story or weak company transmission

### 60. 6531.TW 愛普* — WAIT

- **WHY:** WHY unresolved: obtain company evidence before taking risk
- **Driver:** UNMAPPED / WHY?
- **Driver state:** UNVERIFIED
- **Company transmission:** Unverified
- **International confirmation:** Unverified — same-driver evidence required
- **Relative:** UNVERIFIED
- **Regime:** UNKNOWN
- **Technical state:** Early strength / support recovery
- **Why price:** Wait for a supported pullback with >=2R to observed resistance and <=8% stop distance
- **Entry:** 973–988
- **Invalidation:** Exit on loss of 863; thesis failure also invalidates
- **Add trigger:** Close above 1085 with volume >=1.2x prior median; thesis still supported and recheck R/R
- **Main counter-evidence:** Unverified
- **Main risk:** Unverified causal interpretation
- **What would make us wrong:** A price-only story or weak company transmission

### 61. 2033.TW 佳大 — WAIT

- **WHY:** WHY unresolved: obtain company evidence before taking risk
- **Driver:** UNMAPPED / WHY?
- **Driver state:** UNVERIFIED
- **Company transmission:** Unverified
- **International confirmation:** Unverified — same-driver evidence required
- **Relative:** UNVERIFIED
- **Regime:** UNKNOWN
- **Technical state:** Early strength / support recovery
- **Why price:** Wait for a supported pullback with >=2R to observed resistance and <=8% stop distance
- **Entry:** 28–28.35
- **Invalidation:** Exit on loss of 22.95; thesis failure also invalidates
- **Add trigger:** Close above 28.65 with volume >=1.2x prior median; thesis still supported and recheck R/R
- **Main counter-evidence:** Unverified
- **Main risk:** Unverified causal interpretation
- **What would make us wrong:** A price-only story or weak company transmission

### 62. 5876.TW 上海商銀 — WAIT

- **WHY:** WHY unresolved: obtain company evidence before taking risk
- **Driver:** UNMAPPED / WHY?
- **Driver state:** UNVERIFIED
- **Company transmission:** Unverified
- **International confirmation:** Unverified — same-driver evidence required
- **Relative:** UNVERIFIED
- **Regime:** UNKNOWN
- **Technical state:** Early strength / support recovery
- **Why price:** Wait for a supported pullback with >=2R to observed resistance and <=8% stop distance
- **Entry:** 48.4–48.65
- **Invalidation:** Exit on loss of 42.2; thesis failure also invalidates
- **Add trigger:** Close above 49.05 with volume >=1.2x prior median; thesis still supported and recheck R/R
- **Main counter-evidence:** Unverified
- **Main risk:** Unverified causal interpretation
- **What would make us wrong:** A price-only story or weak company transmission

### 63. 7709.TWO 榮田 — WAIT

- **WHY:** WHY unresolved: obtain company evidence before taking risk
- **Driver:** UNMAPPED / WHY?
- **Driver state:** UNVERIFIED
- **Company transmission:** Unverified
- **International confirmation:** Unverified — same-driver evidence required
- **Relative:** UNVERIFIED
- **Regime:** UNKNOWN
- **Technical state:** Early strength / support recovery
- **Why price:** Wait for a supported pullback with >=2R to observed resistance and <=8% stop distance
- **Entry:** 83–84
- **Invalidation:** Exit on loss of 67.5; thesis failure also invalidates
- **Add trigger:** Close above 94.6 with volume >=1.2x prior median; thesis still supported and recheck R/R
- **Main counter-evidence:** Unverified
- **Main risk:** Unverified causal interpretation
- **What would make us wrong:** A price-only story or weak company transmission

### 64. 7714.TWO 創泓科技 — WAIT

- **WHY:** WHY unresolved: obtain company evidence before taking risk
- **Driver:** UNMAPPED / WHY?
- **Driver state:** UNVERIFIED
- **Company transmission:** Unverified
- **International confirmation:** Unverified — same-driver evidence required
- **Relative:** UNVERIFIED
- **Regime:** UNKNOWN
- **Technical state:** Early strength / support recovery
- **Why price:** Wait for a supported pullback with >=2R to observed resistance and <=8% stop distance
- **Entry:** 172–173.5
- **Invalidation:** Exit on loss of 153.5; thesis failure also invalidates
- **Add trigger:** Close above 194.5 with volume >=1.2x prior median; thesis still supported and recheck R/R
- **Main counter-evidence:** Unverified
- **Main risk:** Unverified causal interpretation
- **What would make us wrong:** A price-only story or weak company transmission

### 65. 2834.TW 臺企銀 — WAIT

- **WHY:** WHY unresolved: obtain company evidence before taking risk
- **Driver:** UNMAPPED / WHY?
- **Driver state:** UNVERIFIED
- **Company transmission:** Unverified
- **International confirmation:** Unverified — same-driver evidence required
- **Relative:** UNVERIFIED
- **Regime:** UNKNOWN
- **Technical state:** Early strength / support recovery
- **Why price:** Wait for a supported pullback with >=2R to observed resistance and <=8% stop distance
- **Entry:** 17.95–18
- **Invalidation:** Exit on loss of 16.3; thesis failure also invalidates
- **Add trigger:** Close above 18.15 with volume >=1.2x prior median; thesis still supported and recheck R/R
- **Main counter-evidence:** Unverified
- **Main risk:** Unverified causal interpretation
- **What would make us wrong:** A price-only story or weak company transmission

### 66. 6957.TW 裕慶-KY — WAIT

- **WHY:** WHY unresolved: obtain company evidence before taking risk
- **Driver:** UNMAPPED / WHY?
- **Driver state:** UNVERIFIED
- **Company transmission:** Unverified
- **International confirmation:** Unverified — same-driver evidence required
- **Relative:** UNVERIFIED
- **Regime:** UNKNOWN
- **Technical state:** Early strength / support recovery
- **Why price:** Wait for a supported pullback with >=2R to observed resistance and <=8% stop distance
- **Entry:** 230–231.5
- **Invalidation:** Exit on loss of 212; thesis failure also invalidates
- **Add trigger:** Close above 235 with volume >=1.2x prior median; thesis still supported and recheck R/R
- **Main counter-evidence:** Unverified
- **Main risk:** Unverified causal interpretation
- **What would make us wrong:** A price-only story or weak company transmission

### 67. 9933.TW 中鼎 — WAIT

- **WHY:** WHY unresolved: obtain company evidence before taking risk
- **Driver:** UNMAPPED / WHY?
- **Driver state:** UNVERIFIED
- **Company transmission:** Unverified
- **International confirmation:** Unverified — same-driver evidence required
- **Relative:** UNVERIFIED
- **Regime:** UNKNOWN
- **Technical state:** Early strength / support recovery
- **Why price:** Wait for a supported pullback with >=2R to observed resistance and <=8% stop distance
- **Entry:** 41.5–41.7
- **Invalidation:** Exit on loss of 38.4; thesis failure also invalidates
- **Add trigger:** Close above 42.55 with volume >=1.2x prior median; thesis still supported and recheck R/R
- **Main counter-evidence:** Unverified
- **Main risk:** Unverified causal interpretation
- **What would make us wrong:** A price-only story or weak company transmission

### 68. 2454.TW 聯發科 — WAIT

- **WHY:** WHY unresolved: obtain company evidence before taking risk
- **Driver:** UNMAPPED / WHY?
- **Driver state:** UNVERIFIED
- **Company transmission:** Unverified
- **International confirmation:** Unverified — same-driver evidence required
- **Relative:** UNVERIFIED
- **Regime:** UNKNOWN
- **Technical state:** Early strength / support recovery
- **Why price:** Wait for a supported pullback with >=2R to observed resistance and <=8% stop distance
- **Entry:** 4535–4585
- **Invalidation:** Exit on loss of 3765; thesis failure also invalidates
- **Add trigger:** Close above 4910 with volume >=1.2x prior median; thesis still supported and recheck R/R
- **Main counter-evidence:** Unverified
- **Main risk:** Unverified causal interpretation
- **What would make us wrong:** A price-only story or weak company transmission

### 69. 1309.TW 台達化 — WAIT

- **WHY:** WHY unresolved: obtain company evidence before taking risk
- **Driver:** UNMAPPED / WHY?
- **Driver state:** UNVERIFIED
- **Company transmission:** Unverified
- **International confirmation:** Unverified — same-driver evidence required
- **Relative:** UNVERIFIED
- **Regime:** UNKNOWN
- **Technical state:** Early strength / support recovery
- **Why price:** Wait for a supported pullback with >=2R to observed resistance and <=8% stop distance
- **Entry:** 13.95–14.05
- **Invalidation:** Exit on loss of 12.85; thesis failure also invalidates
- **Add trigger:** Close above 14.6 with volume >=1.2x prior median; thesis still supported and recheck R/R
- **Main counter-evidence:** Unverified
- **Main risk:** Unverified causal interpretation
- **What would make us wrong:** A price-only story or weak company transmission

### 70. 3024.TW 憶聲 — WAIT

- **WHY:** WHY unresolved: obtain company evidence before taking risk
- **Driver:** UNMAPPED / WHY?
- **Driver state:** UNVERIFIED
- **Company transmission:** Unverified
- **International confirmation:** Unverified — same-driver evidence required
- **Relative:** UNVERIFIED
- **Regime:** UNKNOWN
- **Technical state:** No early strength
- **Why price:** Wait for a supported pullback with >=2R to observed resistance and <=8% stop distance
- **Entry:** 14.3–14.35
- **Invalidation:** Exit on loss of 13.55; thesis failure also invalidates
- **Add trigger:** Close above 15.3 with volume >=1.2x prior median; thesis still supported and recheck R/R
- **Main counter-evidence:** Unverified
- **Main risk:** Unverified causal interpretation
- **What would make us wrong:** A price-only story or weak company transmission

### 71. 1721.TW 國慶科技 — WAIT

- **WHY:** WHY unresolved: obtain company evidence before taking risk
- **Driver:** UNMAPPED / WHY?
- **Driver state:** UNVERIFIED
- **Company transmission:** Unverified
- **International confirmation:** Unverified — same-driver evidence required
- **Relative:** UNVERIFIED
- **Regime:** UNKNOWN
- **Technical state:** Early strength / support recovery
- **Why price:** Wait for a supported pullback with >=2R to observed resistance and <=8% stop distance
- **Entry:** 22.1–22.35
- **Invalidation:** Exit on loss of 19.1; thesis failure also invalidates
- **Add trigger:** Close above 27.2 with volume >=1.2x prior median; thesis still supported and recheck R/R
- **Main counter-evidence:** Unverified
- **Main risk:** Unverified causal interpretation
- **What would make us wrong:** A price-only story or weak company transmission

### 72. 2615.TW 萬海 — WAIT

- **WHY:** WHY unresolved: obtain company evidence before taking risk
- **Driver:** Container freight / liner shipping cycle
- **Driver state:** UNVERIFIED
- **Company transmission:** Unverified
- **International confirmation:** Unverified — same-driver evidence required
- **Relative:** UNVERIFIED
- **Regime:** UNKNOWN
- **Technical state:** No early strength
- **Why price:** Wait for a supported pullback with >=2R to observed resistance and <=8% stop distance
- **Entry:** 111.5–112
- **Invalidation:** Exit on loss of 104.5; thesis failure also invalidates
- **Add trigger:** Close above 136.5 with volume >=1.2x prior median; thesis still supported and recheck R/R
- **Main counter-evidence:** Unverified
- **Main risk:** Unverified causal interpretation
- **What would make us wrong:** A price-only story or weak company transmission

### 73. 3034.TW 聯詠 — WAIT

- **WHY:** WHY unresolved: obtain company evidence before taking risk
- **Driver:** UNMAPPED / WHY?
- **Driver state:** UNVERIFIED
- **Company transmission:** Unverified
- **International confirmation:** Unverified — same-driver evidence required
- **Relative:** UNVERIFIED
- **Regime:** UNKNOWN
- **Technical state:** Early strength / support recovery
- **Why price:** Wait for a supported pullback with >=2R to observed resistance and <=8% stop distance
- **Entry:** 543–547
- **Invalidation:** Exit on loss of 514; thesis failure also invalidates
- **Add trigger:** Close above 572 with volume >=1.2x prior median; thesis still supported and recheck R/R
- **Main counter-evidence:** Unverified
- **Main risk:** Unverified causal interpretation
- **What would make us wrong:** A price-only story or weak company transmission

### 74. 2892.TW 第一金 — WAIT

- **WHY:** WHY unresolved: obtain company evidence before taking risk
- **Driver:** UNMAPPED / WHY?
- **Driver state:** UNVERIFIED
- **Company transmission:** Unverified
- **International confirmation:** Unverified — same-driver evidence required
- **Relative:** UNVERIFIED
- **Regime:** UNKNOWN
- **Technical state:** Early strength / support recovery
- **Why price:** Wait for a supported pullback with >=2R to observed resistance and <=8% stop distance
- **Entry:** 38.7–38.9
- **Invalidation:** Exit on loss of 33.2; thesis failure also invalidates
- **Add trigger:** Close above 38.5 with volume >=1.2x prior median; thesis still supported and recheck R/R
- **Main counter-evidence:** Unverified
- **Main risk:** Unverified causal interpretation
- **What would make us wrong:** A price-only story or weak company transmission

### 75. 2820.TW 華票 — WAIT

- **WHY:** WHY unresolved: obtain company evidence before taking risk
- **Driver:** UNMAPPED / WHY?
- **Driver state:** UNVERIFIED
- **Company transmission:** Unverified
- **International confirmation:** Unverified — same-driver evidence required
- **Relative:** UNVERIFIED
- **Regime:** UNKNOWN
- **Technical state:** Confirmed breakout
- **Why price:** Wait for a supported pullback with >=2R to observed resistance and <=8% stop distance
- **Entry:** 17.35–17.35
- **Invalidation:** Exit on loss of 16.35; thesis failure also invalidates
- **Add trigger:** Close above 17.4 with volume >=1.2x prior median; thesis still supported and recheck R/R
- **Main counter-evidence:** Unverified
- **Main risk:** Unverified causal interpretation
- **What would make us wrong:** A price-only story or weak company transmission

### 76. 2890.TW 永豐金 — WAIT

- **WHY:** WHY unresolved: obtain company evidence before taking risk
- **Driver:** UNMAPPED / WHY?
- **Driver state:** UNVERIFIED
- **Company transmission:** Unverified
- **International confirmation:** Unverified — same-driver evidence required
- **Relative:** UNVERIFIED
- **Regime:** UNKNOWN
- **Technical state:** Early strength / support recovery
- **Why price:** Wait for a supported pullback with >=2R to observed resistance and <=8% stop distance
- **Entry:** 43.45–43.7
- **Invalidation:** Exit on loss of 39.1; thesis failure also invalidates
- **Add trigger:** Close above 44.4 with volume >=1.2x prior median; thesis still supported and recheck R/R
- **Main counter-evidence:** Unverified
- **Main risk:** Unverified causal interpretation
- **What would make us wrong:** A price-only story or weak company transmission

### 77. 2812.TW 台中銀 — WAIT

- **WHY:** WHY unresolved: obtain company evidence before taking risk
- **Driver:** UNMAPPED / WHY?
- **Driver state:** UNVERIFIED
- **Company transmission:** Unverified
- **International confirmation:** Unverified — same-driver evidence required
- **Relative:** UNVERIFIED
- **Regime:** UNKNOWN
- **Technical state:** Early strength / support recovery
- **Why price:** Wait for a supported pullback with >=2R to observed resistance and <=8% stop distance
- **Entry:** 19.45–19.5
- **Invalidation:** Exit on loss of 18.65; thesis failure also invalidates
- **Add trigger:** Close above 19.8 with volume >=1.2x prior median; thesis still supported and recheck R/R
- **Main counter-evidence:** Unverified
- **Main risk:** Unverified causal interpretation
- **What would make us wrong:** A price-only story or weak company transmission

### 78. 4119.TW 旭富 — WAIT

- **WHY:** WHY unresolved: obtain company evidence before taking risk
- **Driver:** UNMAPPED / WHY?
- **Driver state:** UNVERIFIED
- **Company transmission:** Unverified
- **International confirmation:** Unverified — same-driver evidence required
- **Relative:** UNVERIFIED
- **Regime:** UNKNOWN
- **Technical state:** Early strength / support recovery
- **Why price:** Wait for a supported pullback with >=2R to observed resistance and <=8% stop distance
- **Entry:** 57.9–58.2
- **Invalidation:** Exit on loss of 51.7; thesis failure also invalidates
- **Add trigger:** Close above 58.9 with volume >=1.2x prior median; thesis still supported and recheck R/R
- **Main counter-evidence:** Unverified
- **Main risk:** Unverified causal interpretation
- **What would make us wrong:** A price-only story or weak company transmission

### 79. 2492.TW 華新科 — WAIT

- **WHY:** WHY unresolved: obtain company evidence before taking risk
- **Driver:** MLCC demand / pricing cycle
- **Driver state:** UNVERIFIED
- **Company transmission:** Unverified
- **International confirmation:** Unverified — same-driver evidence required
- **Relative:** UNVERIFIED
- **Regime:** UNKNOWN
- **Technical state:** Early strength / support recovery
- **Why price:** Wait for a supported pullback with >=2R to observed resistance and <=8% stop distance
- **Entry:** 306–311.5
- **Invalidation:** Exit on loss of 264; thesis failure also invalidates
- **Add trigger:** Close above 350 with volume >=1.2x prior median; thesis still supported and recheck R/R
- **Main counter-evidence:** Unverified
- **Main risk:** Unverified causal interpretation
- **What would make us wrong:** A price-only story or weak company transmission

### 80. 7703.TWO 銳澤 — WAIT

- **WHY:** WHY unresolved: obtain company evidence before taking risk
- **Driver:** UNMAPPED / WHY?
- **Driver state:** UNVERIFIED
- **Company transmission:** Unverified
- **International confirmation:** Unverified — same-driver evidence required
- **Relative:** UNVERIFIED
- **Regime:** UNKNOWN
- **Technical state:** Early strength / support recovery
- **Why price:** Wait for a supported pullback with >=2R to observed resistance and <=8% stop distance
- **Entry:** 198.5–200
- **Invalidation:** Exit on loss of 175; thesis failure also invalidates
- **Add trigger:** Close above 219 with volume >=1.2x prior median; thesis still supported and recheck R/R
- **Main counter-evidence:** Unverified
- **Main risk:** Unverified causal interpretation
- **What would make us wrong:** A price-only story or weak company transmission

### 81. 4105.TWO 東洋 — WAIT

- **WHY:** WHY unresolved: obtain company evidence before taking risk
- **Driver:** UNMAPPED / WHY?
- **Driver state:** UNVERIFIED
- **Company transmission:** Unverified
- **International confirmation:** Unverified — same-driver evidence required
- **Relative:** UNVERIFIED
- **Regime:** UNKNOWN
- **Technical state:** Early strength / support recovery
- **Why price:** Wait for a supported pullback with >=2R to observed resistance and <=8% stop distance
- **Entry:** 77.4–77.5
- **Invalidation:** Exit on loss of 76.1; thesis failure also invalidates
- **Add trigger:** Close above 79.8 with volume >=1.2x prior median; thesis still supported and recheck R/R
- **Main counter-evidence:** Unverified
- **Main risk:** Unverified causal interpretation
- **What would make us wrong:** A price-only story or weak company transmission

### 82. 3016.TW 嘉晶 — WAIT

- **WHY:** WHY unresolved: obtain company evidence before taking risk
- **Driver:** UNMAPPED / WHY?
- **Driver state:** UNVERIFIED
- **Company transmission:** Unverified
- **International confirmation:** Unverified — same-driver evidence required
- **Relative:** UNVERIFIED
- **Regime:** UNKNOWN
- **Technical state:** Early strength / support recovery
- **Why price:** Wait for a supported pullback with >=2R to observed resistance and <=8% stop distance
- **Entry:** 108.5–109.5
- **Invalidation:** Exit on loss of 96; thesis failure also invalidates
- **Add trigger:** Close above 111.5 with volume >=1.2x prior median; thesis still supported and recheck R/R
- **Main counter-evidence:** Unverified
- **Main risk:** Unverified causal interpretation
- **What would make us wrong:** A price-only story or weak company transmission

### 83. 2855.TW 統一證 — WAIT

- **WHY:** WHY unresolved: obtain company evidence before taking risk
- **Driver:** UNMAPPED / WHY?
- **Driver state:** UNVERIFIED
- **Company transmission:** Unverified
- **International confirmation:** Unverified — same-driver evidence required
- **Relative:** UNVERIFIED
- **Regime:** UNKNOWN
- **Technical state:** Early strength / support recovery
- **Why price:** Wait for a supported pullback with >=2R to observed resistance and <=8% stop distance
- **Entry:** 51.7–51.9
- **Invalidation:** Exit on loss of 47.45; thesis failure also invalidates
- **Add trigger:** Close above 52.6 with volume >=1.2x prior median; thesis still supported and recheck R/R
- **Main counter-evidence:** Unverified
- **Main risk:** Unverified causal interpretation
- **What would make us wrong:** A price-only story or weak company transmission

### 84. 6667.TWO 信紘科 — WAIT

- **WHY:** WHY unresolved: obtain company evidence before taking risk
- **Driver:** UNMAPPED / WHY?
- **Driver state:** UNVERIFIED
- **Company transmission:** Unverified
- **International confirmation:** Unverified — same-driver evidence required
- **Relative:** UNVERIFIED
- **Regime:** UNKNOWN
- **Technical state:** Early strength / support recovery
- **Why price:** Wait for a supported pullback with >=2R to observed resistance and <=8% stop distance
- **Entry:** 245.5–247
- **Invalidation:** Exit on loss of 212; thesis failure also invalidates
- **Add trigger:** Close above 263.5 with volume >=1.2x prior median; thesis still supported and recheck R/R
- **Main counter-evidence:** Unverified
- **Main risk:** Unverified causal interpretation
- **What would make us wrong:** A price-only story or weak company transmission

### 85. 7610.TW 聯友金屬-創 — WAIT

- **WHY:** WHY unresolved: obtain company evidence before taking risk
- **Driver:** UNMAPPED / WHY?
- **Driver state:** UNVERIFIED
- **Company transmission:** Unverified
- **International confirmation:** Unverified — same-driver evidence required
- **Relative:** UNVERIFIED
- **Regime:** UNKNOWN
- **Technical state:** No early strength
- **Why price:** Wait for a supported pullback with >=2R to observed resistance and <=8% stop distance
- **Entry:** 1945–1975
- **Invalidation:** Exit on loss of 1410; thesis failure also invalidates
- **Add trigger:** Close above 2205 with volume >=1.2x prior median; thesis still supported and recheck R/R
- **Main counter-evidence:** Unverified
- **Main risk:** Unverified causal interpretation
- **What would make us wrong:** A price-only story or weak company transmission

### 86. 4961.TW 天鈺 — WAIT

- **WHY:** WHY unresolved: obtain company evidence before taking risk
- **Driver:** UNMAPPED / WHY?
- **Driver state:** UNVERIFIED
- **Company transmission:** Unverified
- **International confirmation:** Unverified — same-driver evidence required
- **Relative:** UNVERIFIED
- **Regime:** UNKNOWN
- **Technical state:** Early strength / support recovery
- **Why price:** Wait for a supported pullback with >=2R to observed resistance and <=8% stop distance
- **Entry:** 169–170
- **Invalidation:** Exit on loss of 157; thesis failure also invalidates
- **Add trigger:** Close above 180.5 with volume >=1.2x prior median; thesis still supported and recheck R/R
- **Main counter-evidence:** Unverified
- **Main risk:** Unverified causal interpretation
- **What would make us wrong:** A price-only story or weak company transmission

### 87. 6152.TW 百一 — WAIT

- **WHY:** WHY unresolved: obtain company evidence before taking risk
- **Driver:** UNMAPPED / WHY?
- **Driver state:** UNVERIFIED
- **Company transmission:** Unverified
- **International confirmation:** Unverified — same-driver evidence required
- **Relative:** UNVERIFIED
- **Regime:** UNKNOWN
- **Technical state:** Early strength / support recovery
- **Why price:** Wait for a supported pullback with >=2R to observed resistance and <=8% stop distance
- **Entry:** 14.9–15
- **Invalidation:** Exit on loss of 13.15; thesis failure also invalidates
- **Add trigger:** Close above 15.55 with volume >=1.2x prior median; thesis still supported and recheck R/R
- **Main counter-evidence:** Unverified
- **Main risk:** Unverified causal interpretation
- **What would make us wrong:** A price-only story or weak company transmission

### 88. 6669.TW 緯穎 — WAIT

- **WHY:** WHY unresolved: obtain company evidence before taking risk
- **Driver:** AI server shipment / ODM cycle
- **Driver state:** UNVERIFIED
- **Company transmission:** Unverified
- **International confirmation:** Unverified — same-driver evidence required
- **Relative:** UNVERIFIED
- **Regime:** UNKNOWN
- **Technical state:** No early strength
- **Why price:** Wait for a supported pullback with >=2R to observed resistance and <=8% stop distance
- **Entry:** 2380–2310
- **Invalidation:** Exit on loss of 2230; thesis failure also invalidates
- **Add trigger:** Close above 2915 with volume >=1.2x prior median; thesis still supported and recheck R/R
- **Main counter-evidence:** Unverified
- **Main risk:** Unverified causal interpretation
- **What would make us wrong:** A price-only story or weak company transmission

### 89. 3324.TWO 雙鴻 — WAIT

- **WHY:** WHY unresolved: obtain company evidence before taking risk
- **Driver:** AI server thermal density / liquid cooling
- **Driver state:** UNVERIFIED
- **Company transmission:** Unverified
- **International confirmation:** Unverified — same-driver evidence required
- **Relative:** UNVERIFIED
- **Regime:** UNKNOWN
- **Technical state:** No early strength
- **Why price:** Wait for a supported pullback with >=2R to observed resistance and <=8% stop distance
- **Entry:** 1340–1360
- **Invalidation:** Exit on loss of 1050; thesis failure also invalidates
- **Add trigger:** Close above 1530 with volume >=1.2x prior median; thesis still supported and recheck R/R
- **Main counter-evidence:** Unverified
- **Main risk:** Unverified causal interpretation
- **What would make us wrong:** A price-only story or weak company transmission

### 90. 2540.TW 愛山林 — WAIT

- **WHY:** WHY unresolved: obtain company evidence before taking risk
- **Driver:** UNMAPPED / WHY?
- **Driver state:** UNVERIFIED
- **Company transmission:** Unverified
- **International confirmation:** Unverified — same-driver evidence required
- **Relative:** UNVERIFIED
- **Regime:** UNKNOWN
- **Technical state:** No early strength
- **Why price:** Wait for a supported pullback with >=2R to observed resistance and <=8% stop distance
- **Entry:** 48.8–49.05
- **Invalidation:** Exit on loss of 46.25; thesis failure also invalidates
- **Add trigger:** Close above 50.25 with volume >=1.2x prior median; thesis still supported and recheck R/R
- **Main counter-evidence:** Unverified
- **Main risk:** Unverified causal interpretation
- **What would make us wrong:** A price-only story or weak company transmission

### 91. 6834.TW 天二科技 — WAIT

- **WHY:** WHY unresolved: obtain company evidence before taking risk
- **Driver:** UNMAPPED / WHY?
- **Driver state:** UNVERIFIED
- **Company transmission:** Unverified
- **International confirmation:** Unverified — same-driver evidence required
- **Relative:** UNVERIFIED
- **Regime:** UNKNOWN
- **Technical state:** Early strength / support recovery
- **Why price:** Wait for a supported pullback with >=2R to observed resistance and <=8% stop distance
- **Entry:** 106.5–108
- **Invalidation:** Exit on loss of 85.2; thesis failure also invalidates
- **Add trigger:** Close above 112 with volume >=1.2x prior median; thesis still supported and recheck R/R
- **Main counter-evidence:** Unverified
- **Main risk:** Unverified causal interpretation
- **What would make us wrong:** A price-only story or weak company transmission

### 92. 8040.TWO 九暘 — WAIT

- **WHY:** WHY unresolved: obtain company evidence before taking risk
- **Driver:** UNMAPPED / WHY?
- **Driver state:** UNVERIFIED
- **Company transmission:** Unverified
- **International confirmation:** Unverified — same-driver evidence required
- **Relative:** UNVERIFIED
- **Regime:** UNKNOWN
- **Technical state:** Early strength / support recovery
- **Why price:** Wait for a supported pullback with >=2R to observed resistance and <=8% stop distance
- **Entry:** 76.1–77.1
- **Invalidation:** Exit on loss of 67.2; thesis failure also invalidates
- **Add trigger:** Close above 87.1 with volume >=1.2x prior median; thesis still supported and recheck R/R
- **Main counter-evidence:** Unverified
- **Main risk:** Unverified causal interpretation
- **What would make us wrong:** A price-only story or weak company transmission

### 93. 5209.TWO 新鼎 — WAIT

- **WHY:** WHY unresolved: obtain company evidence before taking risk
- **Driver:** UNMAPPED / WHY?
- **Driver state:** UNVERIFIED
- **Company transmission:** Unverified
- **International confirmation:** Unverified — same-driver evidence required
- **Relative:** UNVERIFIED
- **Regime:** UNKNOWN
- **Technical state:** Confirmed breakout
- **Why price:** Wait for a supported pullback with >=2R to observed resistance and <=8% stop distance
- **Entry:** 172–172.5
- **Invalidation:** Exit on loss of 151; thesis failure also invalidates
- **Add trigger:** Close above 171.5 with volume >=1.2x prior median; thesis still supported and recheck R/R
- **Main counter-evidence:** Unverified
- **Main risk:** Unverified causal interpretation
- **What would make us wrong:** A price-only story or weak company transmission

### 94. 6292.TWO 迅德 — WAIT

- **WHY:** WHY unresolved: obtain company evidence before taking risk
- **Driver:** UNMAPPED / WHY?
- **Driver state:** UNVERIFIED
- **Company transmission:** Unverified
- **International confirmation:** Unverified — same-driver evidence required
- **Relative:** UNVERIFIED
- **Regime:** UNKNOWN
- **Technical state:** Early strength / support recovery
- **Why price:** Wait for a supported pullback with >=2R to observed resistance and <=8% stop distance
- **Entry:** 64.7–65.1
- **Invalidation:** Exit on loss of 59; thesis failure also invalidates
- **Add trigger:** Close above 67.6 with volume >=1.2x prior median; thesis still supported and recheck R/R
- **Main counter-evidence:** Unverified
- **Main risk:** Unverified causal interpretation
- **What would make us wrong:** A price-only story or weak company transmission

### 95. 8064.TWO 東捷 — WAIT

- **WHY:** WHY unresolved: obtain company evidence before taking risk
- **Driver:** UNMAPPED / WHY?
- **Driver state:** UNVERIFIED
- **Company transmission:** Unverified
- **International confirmation:** Unverified — same-driver evidence required
- **Relative:** UNVERIFIED
- **Regime:** UNKNOWN
- **Technical state:** Early strength / support recovery
- **Why price:** Wait for a supported pullback with >=2R to observed resistance and <=8% stop distance
- **Entry:** 113–115
- **Invalidation:** Exit on loss of 95.7; thesis failure also invalidates
- **Add trigger:** Close above 144 with volume >=1.2x prior median; thesis still supported and recheck R/R
- **Main counter-evidence:** Unverified
- **Main risk:** Unverified causal interpretation
- **What would make us wrong:** A price-only story or weak company transmission

### 96. 2880.TW 華南金 — WAIT

- **WHY:** WHY unresolved: obtain company evidence before taking risk
- **Driver:** UNMAPPED / WHY?
- **Driver state:** UNVERIFIED
- **Company transmission:** Unverified
- **International confirmation:** Unverified — same-driver evidence required
- **Relative:** UNVERIFIED
- **Regime:** UNKNOWN
- **Technical state:** Early strength / support recovery
- **Why price:** Wait for a supported pullback with >=2R to observed resistance and <=8% stop distance
- **Entry:** 45.6–45.8
- **Invalidation:** Exit on loss of 39.05; thesis failure also invalidates
- **Add trigger:** Close above 45.9 with volume >=1.2x prior median; thesis still supported and recheck R/R
- **Main counter-evidence:** Unverified
- **Main risk:** Unverified causal interpretation
- **What would make us wrong:** A price-only story or weak company transmission

### 97. 4770.TW 上品 — WAIT

- **WHY:** WHY unresolved: obtain company evidence before taking risk
- **Driver:** UNMAPPED / WHY?
- **Driver state:** UNVERIFIED
- **Company transmission:** Unverified
- **International confirmation:** Unverified — same-driver evidence required
- **Relative:** UNVERIFIED
- **Regime:** UNKNOWN
- **Technical state:** Early strength / support recovery
- **Why price:** Wait for a supported pullback with >=2R to observed resistance and <=8% stop distance
- **Entry:** 229–231
- **Invalidation:** Exit on loss of 207; thesis failure also invalidates
- **Add trigger:** Close above 252.5 with volume >=1.2x prior median; thesis still supported and recheck R/R
- **Main counter-evidence:** Unverified
- **Main risk:** Unverified causal interpretation
- **What would make us wrong:** A price-only story or weak company transmission

### 98. 2360.TW 致茂 — WAIT

- **WHY:** WHY unresolved: obtain company evidence before taking risk
- **Driver:** UNMAPPED / WHY?
- **Driver state:** UNVERIFIED
- **Company transmission:** Unverified
- **International confirmation:** Unverified — same-driver evidence required
- **Relative:** UNVERIFIED
- **Regime:** UNKNOWN
- **Technical state:** Early strength / support recovery
- **Why price:** Wait for a supported pullback with >=2R to observed resistance and <=8% stop distance
- **Entry:** 2130–2160
- **Invalidation:** Exit on loss of 1870; thesis failure also invalidates
- **Add trigger:** Close above 2525 with volume >=1.2x prior median; thesis still supported and recheck R/R
- **Main counter-evidence:** Unverified
- **Main risk:** Unverified causal interpretation
- **What would make us wrong:** A price-only story or weak company transmission

### 99. 3605.TW 宏致 — WAIT

- **WHY:** WHY unresolved: obtain company evidence before taking risk
- **Driver:** UNMAPPED / WHY?
- **Driver state:** UNVERIFIED
- **Company transmission:** Unverified
- **International confirmation:** Unverified — same-driver evidence required
- **Relative:** UNVERIFIED
- **Regime:** UNKNOWN
- **Technical state:** No early strength
- **Why price:** Wait for a supported pullback with >=2R to observed resistance and <=8% stop distance
- **Entry:** 122–122.5
- **Invalidation:** Exit on loss of 110.5; thesis failure also invalidates
- **Add trigger:** Close above 144 with volume >=1.2x prior median; thesis still supported and recheck R/R
- **Main counter-evidence:** Unverified
- **Main risk:** Unverified causal interpretation
- **What would make us wrong:** A price-only story or weak company transmission

### 100. 3017.TW 奇鋐 — WAIT

- **WHY:** WHY unresolved: obtain company evidence before taking risk
- **Driver:** AI server thermal density / liquid cooling
- **Driver state:** UNVERIFIED
- **Company transmission:** Unverified
- **International confirmation:** Unverified — same-driver evidence required
- **Relative:** UNVERIFIED
- **Regime:** UNKNOWN
- **Technical state:** No early strength
- **Why price:** Wait for a supported pullback with >=2R to observed resistance and <=8% stop distance
- **Entry:** 3385–3370
- **Invalidation:** Exit on loss of 3195; thesis failure also invalidates
- **Add trigger:** Close above 3645 with volume >=1.2x prior median; thesis still supported and recheck R/R
- **Main counter-evidence:** Unverified
- **Main risk:** Unverified causal interpretation
- **What would make us wrong:** A price-only story or weak company transmission

### 101. 6214.TW 精誠 — WAIT

- **WHY:** WHY unresolved: obtain company evidence before taking risk
- **Driver:** UNMAPPED / WHY?
- **Driver state:** UNVERIFIED
- **Company transmission:** Unverified
- **International confirmation:** Unverified — same-driver evidence required
- **Relative:** UNVERIFIED
- **Regime:** UNKNOWN
- **Technical state:** Early strength / support recovery
- **Why price:** Wait for a supported pullback with >=2R to observed resistance and <=8% stop distance
- **Entry:** 188–188.5
- **Invalidation:** Exit on loss of 178; thesis failure also invalidates
- **Add trigger:** Close above 189.5 with volume >=1.2x prior median; thesis still supported and recheck R/R
- **Main counter-evidence:** Unverified
- **Main risk:** Unverified causal interpretation
- **What would make us wrong:** A price-only story or weak company transmission

### 102. 2408.TW 南亞科 — WAIT

- **WHY:** WHY unresolved: obtain company evidence before taking risk
- **Driver:** Commodity DRAM pricing / bit demand
- **Driver state:** UNVERIFIED
- **Company transmission:** Unverified
- **International confirmation:** Unverified — same-driver evidence required
- **Relative:** UNVERIFIED
- **Regime:** UNKNOWN
- **Technical state:** No early strength
- **Why price:** Wait for a supported pullback with >=2R to observed resistance and <=8% stop distance
- **Entry:** 492.5–493
- **Invalidation:** Exit on loss of 462.5; thesis failure also invalidates
- **Add trigger:** Close above 577 with volume >=1.2x prior median; thesis still supported and recheck R/R
- **Main counter-evidence:** Unverified
- **Main risk:** Unverified causal interpretation
- **What would make us wrong:** A price-only story or weak company transmission

### 103. 2206.TW 三陽工業 — WAIT

- **WHY:** WHY unresolved: obtain company evidence before taking risk
- **Driver:** UNMAPPED / WHY?
- **Driver state:** UNVERIFIED
- **Company transmission:** Unverified
- **International confirmation:** Unverified — same-driver evidence required
- **Relative:** UNVERIFIED
- **Regime:** UNKNOWN
- **Technical state:** No early strength
- **Why price:** Wait for a supported pullback with >=2R to observed resistance and <=8% stop distance
- **Entry:** 62.9–63
- **Invalidation:** Exit on loss of 61.8; thesis failure also invalidates
- **Add trigger:** Close above 64.9 with volume >=1.2x prior median; thesis still supported and recheck R/R
- **Main counter-evidence:** Unverified
- **Main risk:** Unverified causal interpretation
- **What would make us wrong:** A price-only story or weak company transmission

### 104. 1215.TW 卜蜂 — WAIT

- **WHY:** WHY unresolved: obtain company evidence before taking risk
- **Driver:** UNMAPPED / WHY?
- **Driver state:** UNVERIFIED
- **Company transmission:** Unverified
- **International confirmation:** Unverified — same-driver evidence required
- **Relative:** UNVERIFIED
- **Regime:** UNKNOWN
- **Technical state:** No early strength
- **Why price:** Wait for a supported pullback with >=2R to observed resistance and <=8% stop distance
- **Entry:** 110.5–110.5
- **Invalidation:** Exit on loss of 106.5; thesis failure also invalidates
- **Add trigger:** Close above 112.5 with volume >=1.2x prior median; thesis still supported and recheck R/R
- **Main counter-evidence:** Unverified
- **Main risk:** Unverified causal interpretation
- **What would make us wrong:** A price-only story or weak company transmission

### 105. 2633.TW 台灣高鐵 — WAIT

- **WHY:** WHY unresolved: obtain company evidence before taking risk
- **Driver:** UNMAPPED / WHY?
- **Driver state:** UNVERIFIED
- **Company transmission:** Unverified
- **International confirmation:** Unverified — same-driver evidence required
- **Relative:** UNVERIFIED
- **Regime:** UNKNOWN
- **Technical state:** No early strength
- **Why price:** Wait for a supported pullback with >=2R to observed resistance and <=8% stop distance
- **Entry:** 26.05–26
- **Invalidation:** Exit on loss of 25.8; thesis failure also invalidates
- **Add trigger:** Close above 26.55 with volume >=1.2x prior median; thesis still supported and recheck R/R
- **Main counter-evidence:** Unverified
- **Main risk:** Unverified causal interpretation
- **What would make us wrong:** A price-only story or weak company transmission

### 106. 3693.TWO 營邦 — WAIT

- **WHY:** WHY unresolved: obtain company evidence before taking risk
- **Driver:** AI rack / server system buildout
- **Driver state:** UNVERIFIED
- **Company transmission:** Unverified
- **International confirmation:** Unverified — same-driver evidence required
- **Relative:** UNVERIFIED
- **Regime:** UNKNOWN
- **Technical state:** No early strength
- **Why price:** Wait for a supported pullback with >=2R to observed resistance and <=8% stop distance
- **Entry:** 654–663
- **Invalidation:** Exit on loss of 539; thesis failure also invalidates
- **Add trigger:** Close above 729 with volume >=1.2x prior median; thesis still supported and recheck R/R
- **Main counter-evidence:** Unverified
- **Main risk:** Unverified causal interpretation
- **What would make us wrong:** A price-only story or weak company transmission

### 107. 2480.TW 敦陽科 — WAIT

- **WHY:** WHY unresolved: obtain company evidence before taking risk
- **Driver:** UNMAPPED / WHY?
- **Driver state:** UNVERIFIED
- **Company transmission:** Unverified
- **International confirmation:** Unverified — same-driver evidence required
- **Relative:** UNVERIFIED
- **Regime:** UNKNOWN
- **Technical state:** Early strength / support recovery
- **Why price:** Wait for a supported pullback with >=2R to observed resistance and <=8% stop distance
- **Entry:** 157–157.5
- **Invalidation:** Exit on loss of 152.5; thesis failure also invalidates
- **Add trigger:** Close above 164.5 with volume >=1.2x prior median; thesis still supported and recheck R/R
- **Main counter-evidence:** Unverified
- **Main risk:** Unverified causal interpretation
- **What would make us wrong:** A price-only story or weak company transmission

### 108. 6116.TW 彩晶 — WAIT

- **WHY:** WHY unresolved: obtain company evidence before taking risk
- **Driver:** UNMAPPED / WHY?
- **Driver state:** UNVERIFIED
- **Company transmission:** Unverified
- **International confirmation:** Unverified — same-driver evidence required
- **Relative:** UNVERIFIED
- **Regime:** UNKNOWN
- **Technical state:** No early strength
- **Why price:** Wait for a supported pullback with >=2R to observed resistance and <=8% stop distance
- **Entry:** 14.3–14.1
- **Invalidation:** Exit on loss of 13.65; thesis failure also invalidates
- **Add trigger:** Close above 15.35 with volume >=1.2x prior median; thesis still supported and recheck R/R
- **Main counter-evidence:** Unverified
- **Main risk:** Unverified causal interpretation
- **What would make us wrong:** A price-only story or weak company transmission

### 109. 1569.TWO 濱川 — WAIT

- **WHY:** WHY unresolved: obtain company evidence before taking risk
- **Driver:** UNMAPPED / WHY?
- **Driver state:** UNVERIFIED
- **Company transmission:** Unverified
- **International confirmation:** Unverified — same-driver evidence required
- **Relative:** UNVERIFIED
- **Regime:** UNKNOWN
- **Technical state:** Early strength / support recovery
- **Why price:** Wait for a supported pullback with >=2R to observed resistance and <=8% stop distance
- **Entry:** 50.2–50.5
- **Invalidation:** Exit on loss of 44.15; thesis failure also invalidates
- **Add trigger:** Close above 51.7 with volume >=1.2x prior median; thesis still supported and recheck R/R
- **Main counter-evidence:** Unverified
- **Main risk:** Unverified causal interpretation
- **What would make us wrong:** A price-only story or weak company transmission

### 110. 2468.TW 華經 — WAIT

- **WHY:** WHY unresolved: obtain company evidence before taking risk
- **Driver:** UNMAPPED / WHY?
- **Driver state:** UNVERIFIED
- **Company transmission:** Unverified
- **International confirmation:** Unverified — same-driver evidence required
- **Relative:** UNVERIFIED
- **Regime:** UNKNOWN
- **Technical state:** Early strength / support recovery
- **Why price:** Wait for a supported pullback with >=2R to observed resistance and <=8% stop distance
- **Entry:** 35.65–35.75
- **Invalidation:** Exit on loss of 31.95; thesis failure also invalidates
- **Add trigger:** Close above 37.4 with volume >=1.2x prior median; thesis still supported and recheck R/R
- **Main counter-evidence:** Unverified
- **Main risk:** Unverified causal interpretation
- **What would make us wrong:** A price-only story or weak company transmission

### 111. 1558.TW 伸興 — WAIT

- **WHY:** WHY unresolved: obtain company evidence before taking risk
- **Driver:** UNMAPPED / WHY?
- **Driver state:** UNVERIFIED
- **Company transmission:** Unverified
- **International confirmation:** Unverified — same-driver evidence required
- **Relative:** UNVERIFIED
- **Regime:** UNKNOWN
- **Technical state:** Early strength / support recovery
- **Why price:** Wait for a supported pullback with >=2R to observed resistance and <=8% stop distance
- **Entry:** 108–108.5
- **Invalidation:** Exit on loss of 99.5; thesis failure also invalidates
- **Add trigger:** Close above 109.5 with volume >=1.2x prior median; thesis still supported and recheck R/R
- **Main counter-evidence:** Unverified
- **Main risk:** Unverified causal interpretation
- **What would make us wrong:** A price-only story or weak company transmission

### 112. 1409.TW 新纖 — WAIT

- **WHY:** WHY unresolved: obtain company evidence before taking risk
- **Driver:** UNMAPPED / WHY?
- **Driver state:** UNVERIFIED
- **Company transmission:** Unverified
- **International confirmation:** Unverified — same-driver evidence required
- **Relative:** UNVERIFIED
- **Regime:** UNKNOWN
- **Technical state:** No early strength
- **Why price:** Wait for a supported pullback with >=2R to observed resistance and <=8% stop distance
- **Entry:** 24.6–24.45
- **Invalidation:** Exit on loss of 23.35; thesis failure also invalidates
- **Add trigger:** Close above 27.4 with volume >=1.2x prior median; thesis still supported and recheck R/R
- **Main counter-evidence:** Unverified
- **Main risk:** Unverified causal interpretation
- **What would make us wrong:** A price-only story or weak company transmission

### 113. 2436.TW 偉詮電 — WAIT

- **WHY:** WHY unresolved: obtain company evidence before taking risk
- **Driver:** UNMAPPED / WHY?
- **Driver state:** UNVERIFIED
- **Company transmission:** Unverified
- **International confirmation:** Unverified — same-driver evidence required
- **Relative:** UNVERIFIED
- **Regime:** UNKNOWN
- **Technical state:** No early strength
- **Why price:** Wait for a supported pullback with >=2R to observed resistance and <=8% stop distance
- **Entry:** 66.4–67
- **Invalidation:** Exit on loss of 63.3; thesis failure also invalidates
- **Add trigger:** Close above 73.8 with volume >=1.2x prior median; thesis still supported and recheck R/R
- **Main counter-evidence:** Unverified
- **Main risk:** Unverified causal interpretation
- **What would make us wrong:** A price-only story or weak company transmission

### 114. 3026.TW 禾伸堂 — WAIT

- **WHY:** WHY unresolved: obtain company evidence before taking risk
- **Driver:** MLCC demand / pricing cycle
- **Driver state:** UNVERIFIED
- **Company transmission:** Unverified
- **International confirmation:** Unverified — same-driver evidence required
- **Relative:** UNVERIFIED
- **Regime:** UNKNOWN
- **Technical state:** Early strength / support recovery
- **Why price:** Wait for a supported pullback with >=2R to observed resistance and <=8% stop distance
- **Entry:** 730–744
- **Invalidation:** Exit on loss of 617; thesis failure also invalidates
- **Add trigger:** Close above 841 with volume >=1.2x prior median; thesis still supported and recheck R/R
- **Main counter-evidence:** Unverified
- **Main risk:** Unverified causal interpretation
- **What would make us wrong:** A price-only story or weak company transmission

### 115. 4904.TW 遠傳 — WAIT

- **WHY:** WHY unresolved: obtain company evidence before taking risk
- **Driver:** UNMAPPED / WHY?
- **Driver state:** UNVERIFIED
- **Company transmission:** Unverified
- **International confirmation:** Unverified — same-driver evidence required
- **Relative:** UNVERIFIED
- **Regime:** UNKNOWN
- **Technical state:** Early strength / support recovery
- **Why price:** Wait for a supported pullback with >=2R to observed resistance and <=8% stop distance
- **Entry:** 105.5–105.5
- **Invalidation:** Exit on loss of 99.3; thesis failure also invalidates
- **Add trigger:** Close above 105.5 with volume >=1.2x prior median; thesis still supported and recheck R/R
- **Main counter-evidence:** Unverified
- **Main risk:** Unverified causal interpretation
- **What would make us wrong:** A price-only story or weak company transmission

### 116. 2382.TW 廣達 — WAIT

- **WHY:** WHY unresolved: obtain company evidence before taking risk
- **Driver:** AI server shipment / ODM cycle
- **Driver state:** UNVERIFIED
- **Company transmission:** Unverified
- **International confirmation:** Unverified — same-driver evidence required
- **Relative:** UNVERIFIED
- **Regime:** UNKNOWN
- **Technical state:** No early strength
- **Why price:** Wait for a supported pullback with >=2R to observed resistance and <=8% stop distance
- **Entry:** 334–336.5
- **Invalidation:** Exit on loss of 323; thesis failure also invalidates
- **Add trigger:** Close above 354.5 with volume >=1.2x prior median; thesis still supported and recheck R/R
- **Main counter-evidence:** Unverified
- **Main risk:** Unverified causal interpretation
- **What would make us wrong:** A price-only story or weak company transmission

### 117. 3231.TW 緯創 — WAIT

- **WHY:** WHY unresolved: obtain company evidence before taking risk
- **Driver:** AI server shipment / ODM cycle
- **Driver state:** UNVERIFIED
- **Company transmission:** Unverified
- **International confirmation:** Unverified — same-driver evidence required
- **Relative:** UNVERIFIED
- **Regime:** UNKNOWN
- **Technical state:** No early strength
- **Why price:** Wait for a supported pullback with >=2R to observed resistance and <=8% stop distance
- **Entry:** 183.5–185.5
- **Invalidation:** Exit on loss of 171.5; thesis failure also invalidates
- **Add trigger:** Close above 208.5 with volume >=1.2x prior median; thesis still supported and recheck R/R
- **Main counter-evidence:** Unverified
- **Main risk:** Unverified causal interpretation
- **What would make us wrong:** A price-only story or weak company transmission

### 118. 3380.TW 明泰 — WAIT

- **WHY:** WHY unresolved: obtain company evidence before taking risk
- **Driver:** UNMAPPED / WHY?
- **Driver state:** UNVERIFIED
- **Company transmission:** Unverified
- **International confirmation:** Unverified — same-driver evidence required
- **Relative:** UNVERIFIED
- **Regime:** UNKNOWN
- **Technical state:** Early strength / support recovery
- **Why price:** Wait for a supported pullback with >=2R to observed resistance and <=8% stop distance
- **Entry:** 30.5–30.75
- **Invalidation:** Exit on loss of 27.5; thesis failure also invalidates
- **Add trigger:** Close above 31.5 with volume >=1.2x prior median; thesis still supported and recheck R/R
- **Main counter-evidence:** Unverified
- **Main risk:** Unverified causal interpretation
- **What would make us wrong:** A price-only story or weak company transmission

### 119. 7795.TW 長廣 — WAIT

- **WHY:** WHY unresolved: obtain company evidence before taking risk
- **Driver:** UNMAPPED / WHY?
- **Driver state:** UNVERIFIED
- **Company transmission:** Unverified
- **International confirmation:** Unverified — same-driver evidence required
- **Relative:** UNVERIFIED
- **Regime:** UNKNOWN
- **Technical state:** No early strength
- **Why price:** Wait for a supported pullback with >=2R to observed resistance and <=8% stop distance
- **Entry:** 524–507
- **Invalidation:** Exit on loss of 477; thesis failure also invalidates
- **Add trigger:** Close above 622 with volume >=1.2x prior median; thesis still supported and recheck R/R
- **Main counter-evidence:** Unverified
- **Main risk:** Unverified causal interpretation
- **What would make us wrong:** A price-only story or weak company transmission

### 120. 1612.TW 宏泰 — WAIT

- **WHY:** WHY unresolved: obtain company evidence before taking risk
- **Driver:** UNMAPPED / WHY?
- **Driver state:** UNVERIFIED
- **Company transmission:** Unverified
- **International confirmation:** Unverified — same-driver evidence required
- **Relative:** UNVERIFIED
- **Regime:** UNKNOWN
- **Technical state:** No early strength
- **Why price:** Wait for a supported pullback with >=2R to observed resistance and <=8% stop distance
- **Entry:** 34.9–34.8
- **Invalidation:** Exit on loss of 34.4; thesis failure also invalidates
- **Add trigger:** Close above 36.05 with volume >=1.2x prior median; thesis still supported and recheck R/R
- **Main counter-evidence:** Unverified
- **Main risk:** Unverified causal interpretation
- **What would make us wrong:** A price-only story or weak company transmission

### 121. 6207.TWO 雷科 — WAIT

- **WHY:** WHY unresolved: obtain company evidence before taking risk
- **Driver:** UNMAPPED / WHY?
- **Driver state:** UNVERIFIED
- **Company transmission:** Unverified
- **International confirmation:** Unverified — same-driver evidence required
- **Relative:** UNVERIFIED
- **Regime:** UNKNOWN
- **Technical state:** Early strength / support recovery
- **Why price:** Wait for a supported pullback with >=2R to observed resistance and <=8% stop distance
- **Entry:** 110.5–112
- **Invalidation:** Exit on loss of 101; thesis failure also invalidates
- **Add trigger:** Close above 125 with volume >=1.2x prior median; thesis still supported and recheck R/R
- **Main counter-evidence:** Unverified
- **Main risk:** Unverified causal interpretation
- **What would make us wrong:** A price-only story or weak company transmission

### 122. 6026.TWO 福邦證 — WAIT

- **WHY:** WHY unresolved: obtain company evidence before taking risk
- **Driver:** UNMAPPED / WHY?
- **Driver state:** UNVERIFIED
- **Company transmission:** Unverified
- **International confirmation:** Unverified — same-driver evidence required
- **Relative:** UNVERIFIED
- **Regime:** UNKNOWN
- **Technical state:** No early strength
- **Why price:** Wait for a supported pullback with >=2R to observed resistance and <=8% stop distance
- **Entry:** 15.05–15.1
- **Invalidation:** Exit on loss of 14.75; thesis failure also invalidates
- **Add trigger:** Close above 15.55 with volume >=1.2x prior median; thesis still supported and recheck R/R
- **Main counter-evidence:** Unverified
- **Main risk:** Unverified causal interpretation
- **What would make us wrong:** A price-only story or weak company transmission

### 123. 9917.TW 中保科 — WAIT

- **WHY:** WHY unresolved: obtain company evidence before taking risk
- **Driver:** UNMAPPED / WHY?
- **Driver state:** UNVERIFIED
- **Company transmission:** Unverified
- **International confirmation:** Unverified — same-driver evidence required
- **Relative:** UNVERIFIED
- **Regime:** UNKNOWN
- **Technical state:** No early strength
- **Why price:** Wait for a supported pullback with >=2R to observed resistance and <=8% stop distance
- **Entry:** 105.5–105.5
- **Invalidation:** Exit on loss of 104.5; thesis failure also invalidates
- **Add trigger:** Close above 107.5 with volume >=1.2x prior median; thesis still supported and recheck R/R
- **Main counter-evidence:** Unverified
- **Main risk:** Unverified causal interpretation
- **What would make us wrong:** A price-only story or weak company transmission

### 124. 6510.TWO 精測 — WAIT

- **WHY:** WHY unresolved: obtain company evidence before taking risk
- **Driver:** Advanced packaging / test capex
- **Driver state:** UNVERIFIED
- **Company transmission:** Unverified
- **International confirmation:** Unverified — same-driver evidence required
- **Relative:** UNVERIFIED
- **Regime:** UNKNOWN
- **Technical state:** Early strength / support recovery
- **Why price:** Wait for a supported pullback with >=2R to observed resistance and <=8% stop distance
- **Entry:** 3420–3480
- **Invalidation:** Exit on loss of 2545; thesis failure also invalidates
- **Add trigger:** Close above 3995 with volume >=1.2x prior median; thesis still supported and recheck R/R
- **Main counter-evidence:** Unverified
- **Main risk:** Unverified causal interpretation
- **What would make us wrong:** A price-only story or weak company transmission

### 125. 6291.TWO 沛亨 — WAIT

- **WHY:** WHY unresolved: obtain company evidence before taking risk
- **Driver:** UNMAPPED / WHY?
- **Driver state:** UNVERIFIED
- **Company transmission:** Unverified
- **International confirmation:** Unverified — same-driver evidence required
- **Relative:** UNVERIFIED
- **Regime:** UNKNOWN
- **Technical state:** Early strength / support recovery
- **Why price:** Wait for a supported pullback with >=2R to observed resistance and <=8% stop distance
- **Entry:** 470–478
- **Invalidation:** Exit on loss of 379.5; thesis failure also invalidates
- **Add trigger:** Close above 515 with volume >=1.2x prior median; thesis still supported and recheck R/R
- **Main counter-evidence:** Unverified
- **Main risk:** Unverified causal interpretation
- **What would make us wrong:** A price-only story or weak company transmission

### 126. 1304.TW 台聚 — WAIT

- **WHY:** WHY unresolved: obtain company evidence before taking risk
- **Driver:** UNMAPPED / WHY?
- **Driver state:** UNVERIFIED
- **Company transmission:** Unverified
- **International confirmation:** Unverified — same-driver evidence required
- **Relative:** UNVERIFIED
- **Regime:** UNKNOWN
- **Technical state:** No early strength
- **Why price:** Wait for a supported pullback with >=2R to observed resistance and <=8% stop distance
- **Entry:** 12.4–12.35
- **Invalidation:** Exit on loss of 11.85; thesis failure also invalidates
- **Add trigger:** Close above 13.3 with volume >=1.2x prior median; thesis still supported and recheck R/R
- **Main counter-evidence:** Unverified
- **Main risk:** Unverified causal interpretation
- **What would make us wrong:** A price-only story or weak company transmission

### 127. 1313.TW 聯成 — WAIT

- **WHY:** WHY unresolved: obtain company evidence before taking risk
- **Driver:** UNMAPPED / WHY?
- **Driver state:** UNVERIFIED
- **Company transmission:** Unverified
- **International confirmation:** Unverified — same-driver evidence required
- **Relative:** UNVERIFIED
- **Regime:** UNKNOWN
- **Technical state:** Early strength / support recovery
- **Why price:** Wait for a supported pullback with >=2R to observed resistance and <=8% stop distance
- **Entry:** 12–11.9
- **Invalidation:** Exit on loss of 11.55; thesis failure also invalidates
- **Add trigger:** Close above 12.75 with volume >=1.2x prior median; thesis still supported and recheck R/R
- **Main counter-evidence:** Unverified
- **Main risk:** Unverified causal interpretation
- **What would make us wrong:** A price-only story or weak company transmission

### 128. 2504.TW 國產 — WAIT

- **WHY:** WHY unresolved: obtain company evidence before taking risk
- **Driver:** UNMAPPED / WHY?
- **Driver state:** UNVERIFIED
- **Company transmission:** Unverified
- **International confirmation:** Unverified — same-driver evidence required
- **Relative:** UNVERIFIED
- **Regime:** UNKNOWN
- **Technical state:** No early strength
- **Why price:** Wait for a supported pullback with >=2R to observed resistance and <=8% stop distance
- **Entry:** 31.45–31.25
- **Invalidation:** Exit on loss of 30.95; thesis failure also invalidates
- **Add trigger:** Close above 32.7 with volume >=1.2x prior median; thesis still supported and recheck R/R
- **Main counter-evidence:** Unverified
- **Main risk:** Unverified causal interpretation
- **What would make us wrong:** A price-only story or weak company transmission

### 129. 6670.TW 復盛應用 — WAIT

- **WHY:** WHY unresolved: obtain company evidence before taking risk
- **Driver:** UNMAPPED / WHY?
- **Driver state:** UNVERIFIED
- **Company transmission:** Unverified
- **International confirmation:** Unverified — same-driver evidence required
- **Relative:** UNVERIFIED
- **Regime:** UNKNOWN
- **Technical state:** No early strength
- **Why price:** Wait for a supported pullback with >=2R to observed resistance and <=8% stop distance
- **Entry:** 275–275
- **Invalidation:** Exit on loss of 268; thesis failure also invalidates
- **Add trigger:** Close above 290 with volume >=1.2x prior median; thesis still supported and recheck R/R
- **Main counter-evidence:** Unverified
- **Main risk:** Unverified causal interpretation
- **What would make us wrong:** A price-only story or weak company transmission

### 130. 1305.TW 華夏 — WAIT

- **WHY:** WHY unresolved: obtain company evidence before taking risk
- **Driver:** UNMAPPED / WHY?
- **Driver state:** UNVERIFIED
- **Company transmission:** Unverified
- **International confirmation:** Unverified — same-driver evidence required
- **Relative:** UNVERIFIED
- **Regime:** UNKNOWN
- **Technical state:** No early strength
- **Why price:** Wait for a supported pullback with >=2R to observed resistance and <=8% stop distance
- **Entry:** 12.3–12.1
- **Invalidation:** Exit on loss of 11.8; thesis failure also invalidates
- **Add trigger:** Close above 13.25 with volume >=1.2x prior median; thesis still supported and recheck R/R
- **Main counter-evidence:** Unverified
- **Main risk:** Unverified causal interpretation
- **What would make us wrong:** A price-only story or weak company transmission

### 131. 3029.TW 零壹 — WAIT

- **WHY:** WHY unresolved: obtain company evidence before taking risk
- **Driver:** Enterprise cybersecurity spending
- **Driver state:** UNVERIFIED
- **Company transmission:** Unverified
- **International confirmation:** Unverified — same-driver evidence required
- **Relative:** UNVERIFIED
- **Regime:** UNKNOWN
- **Technical state:** Confirmed breakout
- **Why price:** Wait for a supported pullback with >=2R to observed resistance and <=8% stop distance
- **Entry:** 116–116.5
- **Invalidation:** Exit on loss of 107.5; thesis failure also invalidates
- **Add trigger:** Close above 116.5 with volume >=1.2x prior median; thesis still supported and recheck R/R
- **Main counter-evidence:** Unverified
- **Main risk:** Unverified causal interpretation
- **What would make us wrong:** A price-only story or weak company transmission

### 132. 3005.TW 神基 — WAIT

- **WHY:** WHY unresolved: obtain company evidence before taking risk
- **Driver:** UNMAPPED / WHY?
- **Driver state:** UNVERIFIED
- **Company transmission:** Unverified
- **International confirmation:** Unverified — same-driver evidence required
- **Relative:** UNVERIFIED
- **Regime:** UNKNOWN
- **Technical state:** No early strength
- **Why price:** Wait for a supported pullback with >=2R to observed resistance and <=8% stop distance
- **Entry:** 115–115.5
- **Invalidation:** Exit on loss of 111; thesis failure also invalidates
- **Add trigger:** Close above 126 with volume >=1.2x prior median; thesis still supported and recheck R/R
- **Main counter-evidence:** Unverified
- **Main risk:** Unverified causal interpretation
- **What would make us wrong:** A price-only story or weak company transmission

### 133. 2891.TW 中信金 — WAIT

- **WHY:** WHY unresolved: obtain company evidence before taking risk
- **Driver:** Financials rate / credit / margin cycle
- **Driver state:** UNVERIFIED
- **Company transmission:** Unverified
- **International confirmation:** Unverified — same-driver evidence required
- **Relative:** UNVERIFIED
- **Regime:** UNKNOWN
- **Technical state:** No early strength
- **Why price:** Wait for a supported pullback with >=2R to observed resistance and <=8% stop distance
- **Entry:** 66.8–67
- **Invalidation:** Exit on loss of 63.1; thesis failure also invalidates
- **Add trigger:** Close above 69.9 with volume >=1.2x prior median; thesis still supported and recheck R/R
- **Main counter-evidence:** Unverified
- **Main risk:** Unverified causal interpretation
- **What would make us wrong:** A price-only story or weak company transmission

### 134. 2912.TW 統一超 — WAIT

- **WHY:** WHY unresolved: obtain company evidence before taking risk
- **Driver:** UNMAPPED / WHY?
- **Driver state:** UNVERIFIED
- **Company transmission:** Unverified
- **International confirmation:** Unverified — same-driver evidence required
- **Relative:** UNVERIFIED
- **Regime:** UNKNOWN
- **Technical state:** No early strength
- **Why price:** Wait for a supported pullback with >=2R to observed resistance and <=8% stop distance
- **Entry:** 216–216.5
- **Invalidation:** Exit on loss of 213; thesis failure also invalidates
- **Add trigger:** Close above 226.5 with volume >=1.2x prior median; thesis still supported and recheck R/R
- **Main counter-evidence:** Unverified
- **Main risk:** Unverified causal interpretation
- **What would make us wrong:** A price-only story or weak company transmission

### 135. 6257.TW 矽格 — WAIT

- **WHY:** WHY unresolved: obtain company evidence before taking risk
- **Driver:** UNMAPPED / WHY?
- **Driver state:** UNVERIFIED
- **Company transmission:** Unverified
- **International confirmation:** Unverified — same-driver evidence required
- **Relative:** UNVERIFIED
- **Regime:** UNKNOWN
- **Technical state:** No early strength
- **Why price:** Wait for a supported pullback with >=2R to observed resistance and <=8% stop distance
- **Entry:** 209.5–211.5
- **Invalidation:** Exit on loss of 199; thesis failure also invalidates
- **Add trigger:** Close above 231 with volume >=1.2x prior median; thesis still supported and recheck R/R
- **Main counter-evidence:** Unverified
- **Main risk:** Unverified causal interpretation
- **What would make us wrong:** A price-only story or weak company transmission

### 136. 4590.TW 富田-創 — WAIT

- **WHY:** WHY unresolved: obtain company evidence before taking risk
- **Driver:** UNMAPPED / WHY?
- **Driver state:** UNVERIFIED
- **Company transmission:** Unverified
- **International confirmation:** Unverified — same-driver evidence required
- **Relative:** UNVERIFIED
- **Regime:** UNKNOWN
- **Technical state:** No early strength
- **Why price:** Wait for a supported pullback with >=2R to observed resistance and <=8% stop distance
- **Entry:** 89.7–89.9
- **Invalidation:** Exit on loss of 86.6; thesis failure also invalidates
- **Add trigger:** Close above 100.3 with volume >=1.2x prior median; thesis still supported and recheck R/R
- **Main counter-evidence:** Unverified
- **Main risk:** Unverified causal interpretation
- **What would make us wrong:** A price-only story or weak company transmission

### 137. 7728.TWO 光焱科技 — WAIT

- **WHY:** WHY unresolved: obtain company evidence before taking risk
- **Driver:** UNMAPPED / WHY?
- **Driver state:** UNVERIFIED
- **Company transmission:** Unverified
- **International confirmation:** Unverified — same-driver evidence required
- **Relative:** UNVERIFIED
- **Regime:** UNKNOWN
- **Technical state:** No early strength
- **Why price:** Wait for a supported pullback with >=2R to observed resistance and <=8% stop distance
- **Entry:** 625–616
- **Invalidation:** Exit on loss of 593; thesis failure also invalidates
- **Add trigger:** Close above 696 with volume >=1.2x prior median; thesis still supported and recheck R/R
- **Main counter-evidence:** Unverified
- **Main risk:** Unverified causal interpretation
- **What would make us wrong:** A price-only story or weak company transmission

### 138. 1314.TW 中石化 — WAIT

- **WHY:** WHY unresolved: obtain company evidence before taking risk
- **Driver:** UNMAPPED / WHY?
- **Driver state:** UNVERIFIED
- **Company transmission:** Unverified
- **International confirmation:** Unverified — same-driver evidence required
- **Relative:** UNVERIFIED
- **Regime:** UNKNOWN
- **Technical state:** No early strength
- **Why price:** Wait for a supported pullback with >=2R to observed resistance and <=8% stop distance
- **Entry:** 8.16–8.11
- **Invalidation:** Exit on loss of 7.94; thesis failure also invalidates
- **Add trigger:** Close above 8.61 with volume >=1.2x prior median; thesis still supported and recheck R/R
- **Main counter-evidence:** Unverified
- **Main risk:** Unverified causal interpretation
- **What would make us wrong:** A price-only story or weak company transmission

### 139. 7828.TWO 創新服務 — WAIT

- **WHY:** WHY unresolved: obtain company evidence before taking risk
- **Driver:** UNMAPPED / WHY?
- **Driver state:** UNVERIFIED
- **Company transmission:** Unverified
- **International confirmation:** Unverified — same-driver evidence required
- **Relative:** UNVERIFIED
- **Regime:** UNKNOWN
- **Technical state:** Early strength / support recovery
- **Why price:** Wait for a supported pullback with >=2R to observed resistance and <=8% stop distance
- **Entry:** 1940–1975
- **Invalidation:** Exit on loss of 1595; thesis failure also invalidates
- **Add trigger:** Close above 2270 with volume >=1.2x prior median; thesis still supported and recheck R/R
- **Main counter-evidence:** Unverified
- **Main risk:** Unverified causal interpretation
- **What would make us wrong:** A price-only story or weak company transmission

### 140. 9907.TW 統一實 — WAIT

- **WHY:** WHY unresolved: obtain company evidence before taking risk
- **Driver:** UNMAPPED / WHY?
- **Driver state:** UNVERIFIED
- **Company transmission:** Unverified
- **International confirmation:** Unverified — same-driver evidence required
- **Relative:** UNVERIFIED
- **Regime:** UNKNOWN
- **Technical state:** No early strength
- **Why price:** Wait for a supported pullback with >=2R to observed resistance and <=8% stop distance
- **Entry:** 15.3–15.1
- **Invalidation:** Exit on loss of 15.05; thesis failure also invalidates
- **Add trigger:** Close above 15.7 with volume >=1.2x prior median; thesis still supported and recheck R/R
- **Main counter-evidence:** Unverified
- **Main risk:** Unverified causal interpretation
- **What would make us wrong:** A price-only story or weak company transmission

### 141. 2428.TW 興勤 — WAIT

- **WHY:** WHY unresolved: obtain company evidence before taking risk
- **Driver:** UNMAPPED / WHY?
- **Driver state:** UNVERIFIED
- **Company transmission:** Unverified
- **International confirmation:** Unverified — same-driver evidence required
- **Relative:** UNVERIFIED
- **Regime:** UNKNOWN
- **Technical state:** No early strength
- **Why price:** Wait for a supported pullback with >=2R to observed resistance and <=8% stop distance
- **Entry:** 253–255
- **Invalidation:** Exit on loss of 242; thesis failure also invalidates
- **Add trigger:** Close above 284 with volume >=1.2x prior median; thesis still supported and recheck R/R
- **Main counter-evidence:** Unverified
- **Main risk:** Unverified causal interpretation
- **What would make us wrong:** A price-only story or weak company transmission

### 142. 3437.TW 榮創 — WAIT

- **WHY:** WHY unresolved: obtain company evidence before taking risk
- **Driver:** UNMAPPED / WHY?
- **Driver state:** UNVERIFIED
- **Company transmission:** Unverified
- **International confirmation:** Unverified — same-driver evidence required
- **Relative:** UNVERIFIED
- **Regime:** UNKNOWN
- **Technical state:** No early strength
- **Why price:** Wait for a supported pullback with >=2R to observed resistance and <=8% stop distance
- **Entry:** 18.2–18.1
- **Invalidation:** Exit on loss of 17.35; thesis failure also invalidates
- **Add trigger:** Close above 20.45 with volume >=1.2x prior median; thesis still supported and recheck R/R
- **Main counter-evidence:** Unverified
- **Main risk:** Unverified causal interpretation
- **What would make us wrong:** A price-only story or weak company transmission

### 143. 2345.TW 智邦 — WAIT

- **WHY:** WHY unresolved: obtain company evidence before taking risk
- **Driver:** 800G/1.6T AI networking upgrade
- **Driver state:** UNVERIFIED
- **Company transmission:** Unverified
- **International confirmation:** Unverified — same-driver evidence required
- **Relative:** UNVERIFIED
- **Regime:** UNKNOWN
- **Technical state:** No early strength
- **Why price:** Wait for a supported pullback with >=2R to observed resistance and <=8% stop distance
- **Entry:** Unavailable
- **Invalidation:** Exit on loss of 1920; thesis failure also invalidates
- **Add trigger:** Close above 2415 with volume >=1.2x prior median; thesis still supported and recheck R/R
- **Main counter-evidence:** Unverified
- **Main risk:** Unverified causal interpretation
- **What would make us wrong:** A price-only story or weak company transmission

### 144. 5351.TWO 鈺創 — WAIT

- **WHY:** WHY unresolved: obtain company evidence before taking risk
- **Driver:** Memory IC design demand / ASP cycle
- **Driver state:** UNVERIFIED
- **Company transmission:** Unverified
- **International confirmation:** Unverified — same-driver evidence required
- **Relative:** UNVERIFIED
- **Regime:** UNKNOWN
- **Technical state:** No early strength
- **Why price:** Wait for a supported pullback with >=2R to observed resistance and <=8% stop distance
- **Entry:** 112–108.5
- **Invalidation:** Exit on loss of 104.5; thesis failure also invalidates
- **Add trigger:** Close above 143 with volume >=1.2x prior median; thesis still supported and recheck R/R
- **Main counter-evidence:** Unverified
- **Main risk:** Unverified causal interpretation
- **What would make us wrong:** A price-only story or weak company transmission

### 145. 2344.TW 華邦電 — WAIT

- **WHY:** WHY unresolved: obtain company evidence before taking risk
- **Driver:** Specialty DRAM / NOR / NAND pricing
- **Driver state:** UNVERIFIED
- **Company transmission:** Unverified
- **International confirmation:** Unverified — same-driver evidence required
- **Relative:** UNVERIFIED
- **Regime:** UNKNOWN
- **Technical state:** No early strength
- **Why price:** Wait for a supported pullback with >=2R to observed resistance and <=8% stop distance
- **Entry:** 173–171.5
- **Invalidation:** Exit on loss of 163; thesis failure also invalidates
- **Add trigger:** Close above 194.5 with volume >=1.2x prior median; thesis still supported and recheck R/R
- **Main counter-evidence:** Unverified
- **Main risk:** Unverified causal interpretation
- **What would make us wrong:** A price-only story or weak company transmission

### 146. 2337.TW 旺宏 — WAIT

- **WHY:** WHY unresolved: obtain company evidence before taking risk
- **Driver:** Specialty DRAM / NOR / NAND pricing
- **Driver state:** UNVERIFIED
- **Company transmission:** Unverified
- **International confirmation:** Unverified — same-driver evidence required
- **Relative:** UNVERIFIED
- **Regime:** UNKNOWN
- **Technical state:** No early strength
- **Why price:** Wait for a supported pullback with >=2R to observed resistance and <=8% stop distance
- **Entry:** 121–119
- **Invalidation:** Exit on loss of 115; thesis failure also invalidates
- **Add trigger:** Close above 146 with volume >=1.2x prior median; thesis still supported and recheck R/R
- **Main counter-evidence:** Unverified
- **Main risk:** Unverified causal interpretation
- **What would make us wrong:** A price-only story or weak company transmission

### 147. 8299.TWO 群聯 — WAIT

- **WHY:** WHY unresolved: obtain company evidence before taking risk
- **Driver:** NAND / storage controller and module cycle
- **Driver state:** UNVERIFIED
- **Company transmission:** Unverified
- **International confirmation:** Unverified — same-driver evidence required
- **Relative:** UNVERIFIED
- **Regime:** UNKNOWN
- **Technical state:** No early strength
- **Why price:** Wait for a supported pullback with >=2R to observed resistance and <=8% stop distance
- **Entry:** 2035–1990
- **Invalidation:** Exit on loss of 1945; thesis failure also invalidates
- **Add trigger:** Close above 2350 with volume >=1.2x prior median; thesis still supported and recheck R/R
- **Main counter-evidence:** Unverified
- **Main risk:** Unverified causal interpretation
- **What would make us wrong:** A price-only story or weak company transmission

### 148. 2376.TW 技嘉 — WAIT

- **WHY:** WHY unresolved: obtain company evidence before taking risk
- **Driver:** AI rack / server system buildout
- **Driver state:** UNVERIFIED
- **Company transmission:** Unverified
- **International confirmation:** Unverified — same-driver evidence required
- **Relative:** UNVERIFIED
- **Regime:** UNKNOWN
- **Technical state:** No early strength
- **Why price:** Wait for a supported pullback with >=2R to observed resistance and <=8% stop distance
- **Entry:** 350.5–349
- **Invalidation:** Exit on loss of 339.5; thesis failure also invalidates
- **Add trigger:** Close above 405 with volume >=1.2x prior median; thesis still supported and recheck R/R
- **Main counter-evidence:** Unverified
- **Main risk:** Unverified causal interpretation
- **What would make us wrong:** A price-only story or weak company transmission

### 149. 1519.TW 華城 — WAIT

- **WHY:** WHY unresolved: obtain company evidence before taking risk
- **Driver:** Grid modernization / transformer and cable capex
- **Driver state:** UNVERIFIED
- **Company transmission:** Unverified
- **International confirmation:** Unverified — same-driver evidence required
- **Relative:** UNVERIFIED
- **Regime:** UNKNOWN
- **Technical state:** No early strength
- **Why price:** Wait for a supported pullback with >=2R to observed resistance and <=8% stop distance
- **Entry:** 734–712
- **Invalidation:** Exit on loss of 705; thesis failure also invalidates
- **Add trigger:** Close above 794 with volume >=1.2x prior median; thesis still supported and recheck R/R
- **Main counter-evidence:** Unverified
- **Main risk:** Unverified causal interpretation
- **What would make us wrong:** A price-only story or weak company transmission

### 150. 3260.TWO 威剛 — WAIT

- **WHY:** WHY unresolved: obtain company evidence before taking risk
- **Driver:** NAND / storage controller and module cycle
- **Driver state:** UNVERIFIED
- **Company transmission:** Unverified
- **International confirmation:** Unverified — same-driver evidence required
- **Relative:** UNVERIFIED
- **Regime:** UNKNOWN
- **Technical state:** No early strength
- **Why price:** Wait for a supported pullback with >=2R to observed resistance and <=8% stop distance
- **Entry:** 402–402.5
- **Invalidation:** Exit on loss of 386.5; thesis failure also invalidates
- **Add trigger:** Close above 463.5 with volume >=1.2x prior median; thesis still supported and recheck R/R
- **Main counter-evidence:** Unverified
- **Main risk:** Unverified causal interpretation
- **What would make us wrong:** A price-only story or weak company transmission

### 151. 2317.TW 鴻海 — WAIT

- **WHY:** WHY unresolved: obtain company evidence before taking risk
- **Driver:** AI server shipment / ODM cycle
- **Driver state:** UNVERIFIED
- **Company transmission:** Unverified
- **International confirmation:** Unverified — same-driver evidence required
- **Relative:** UNVERIFIED
- **Regime:** UNKNOWN
- **Technical state:** No early strength
- **Why price:** Wait for a supported pullback with >=2R to observed resistance and <=8% stop distance
- **Entry:** 250.5–248
- **Invalidation:** Exit on loss of 244; thesis failure also invalidates
- **Add trigger:** Close above 266.5 with volume >=1.2x prior median; thesis still supported and recheck R/R
- **Main counter-evidence:** Unverified
- **Main risk:** Unverified causal interpretation
- **What would make us wrong:** A price-only story or weak company transmission

### 152. 5289.TWO 宜鼎 — WAIT

- **WHY:** WHY unresolved: obtain company evidence before taking risk
- **Driver:** NAND / storage controller and module cycle
- **Driver state:** UNVERIFIED
- **Company transmission:** Unverified
- **International confirmation:** Unverified — same-driver evidence required
- **Relative:** UNVERIFIED
- **Regime:** UNKNOWN
- **Technical state:** No early strength
- **Why price:** Wait for a supported pullback with >=2R to observed resistance and <=8% stop distance
- **Entry:** 1335–1295
- **Invalidation:** Exit on loss of 1270; thesis failure also invalidates
- **Add trigger:** Close above 1645 with volume >=1.2x prior median; thesis still supported and recheck R/R
- **Main counter-evidence:** Unverified
- **Main risk:** Unverified causal interpretation
- **What would make us wrong:** A price-only story or weak company transmission

### 153. 6640.TWO 均華 — WAIT

- **WHY:** WHY unresolved: obtain company evidence before taking risk
- **Driver:** Advanced packaging / test capex
- **Driver state:** UNVERIFIED
- **Company transmission:** Unverified
- **International confirmation:** Unverified — same-driver evidence required
- **Relative:** UNVERIFIED
- **Regime:** UNKNOWN
- **Technical state:** No early strength
- **Why price:** Wait for a supported pullback with >=2R to observed resistance and <=8% stop distance
- **Entry:** 1150–1170
- **Invalidation:** Exit on loss of 1005; thesis failure also invalidates
- **Add trigger:** Close above 1345 with volume >=1.2x prior median; thesis still supported and recheck R/R
- **Main counter-evidence:** Unverified
- **Main risk:** Unverified causal interpretation
- **What would make us wrong:** A price-only story or weak company transmission

### 154. 1608.TW 華榮 — WAIT

- **WHY:** WHY unresolved: obtain company evidence before taking risk
- **Driver:** Copper commodity price / trade-inventory cycle
- **Driver state:** UNVERIFIED
- **Company transmission:** Unverified
- **International confirmation:** Unverified — same-driver evidence required
- **Relative:** UNVERIFIED
- **Regime:** UNKNOWN
- **Technical state:** No early strength
- **Why price:** Wait for a supported pullback with >=2R to observed resistance and <=8% stop distance
- **Entry:** 36.7–36.2
- **Invalidation:** Exit on loss of 35.65; thesis failure also invalidates
- **Add trigger:** Close above 39.4 with volume >=1.2x prior median; thesis still supported and recheck R/R
- **Main counter-evidence:** Unverified
- **Main risk:** Unverified causal interpretation
- **What would make us wrong:** A price-only story or weak company transmission

### 155. 2356.TW 英業達 — WAIT

- **WHY:** WHY unresolved: obtain company evidence before taking risk
- **Driver:** AI server shipment / ODM cycle
- **Driver state:** UNVERIFIED
- **Company transmission:** Unverified
- **International confirmation:** Unverified — same-driver evidence required
- **Relative:** UNVERIFIED
- **Regime:** UNKNOWN
- **Technical state:** No early strength
- **Why price:** Wait for a supported pullback with >=2R to observed resistance and <=8% stop distance
- **Entry:** 64.3–62
- **Invalidation:** Exit on loss of 61.8; thesis failure also invalidates
- **Add trigger:** Close above 74.2 with volume >=1.2x prior median; thesis still supported and recheck R/R
- **Main counter-evidence:** Unverified
- **Main risk:** Unverified causal interpretation
- **What would make us wrong:** A price-only story or weak company transmission

### 156. 1513.TW 中興電 — WAIT

- **WHY:** WHY unresolved: obtain company evidence before taking risk
- **Driver:** Grid modernization / transformer and cable capex
- **Driver state:** UNVERIFIED
- **Company transmission:** Unverified
- **International confirmation:** Unverified — same-driver evidence required
- **Relative:** UNVERIFIED
- **Regime:** UNKNOWN
- **Technical state:** No early strength
- **Why price:** Wait for a supported pullback with >=2R to observed resistance and <=8% stop distance
- **Entry:** 165.5–165
- **Invalidation:** Exit on loss of 161.5; thesis failure also invalidates
- **Add trigger:** Close above 175 with volume >=1.2x prior median; thesis still supported and recheck R/R
- **Main counter-evidence:** Unverified
- **Main risk:** Unverified causal interpretation
- **What would make us wrong:** A price-only story or weak company transmission

### 157. 6230.TW 尼得科超眾 — WAIT

- **WHY:** WHY unresolved: obtain company evidence before taking risk
- **Driver:** AI server thermal density / liquid cooling
- **Driver state:** UNVERIFIED
- **Company transmission:** Unverified
- **International confirmation:** Unverified — same-driver evidence required
- **Relative:** UNVERIFIED
- **Regime:** UNKNOWN
- **Technical state:** No early strength
- **Why price:** Wait for a supported pullback with >=2R to observed resistance and <=8% stop distance
- **Entry:** 108–106
- **Invalidation:** Exit on loss of 104.5; thesis failure also invalidates
- **Add trigger:** Close above 116 with volume >=1.2x prior median; thesis still supported and recheck R/R
- **Main counter-evidence:** Unverified
- **Main risk:** Unverified causal interpretation
- **What would make us wrong:** A price-only story or weak company transmission

### 158. 5347.TWO 世界 — WAIT

- **WHY:** WHY unresolved: obtain company evidence before taking risk
- **Driver:** Mature-node foundry utilization / pricing
- **Driver state:** UNVERIFIED
- **Company transmission:** Unverified
- **International confirmation:** Unverified — same-driver evidence required
- **Relative:** UNVERIFIED
- **Regime:** UNKNOWN
- **Technical state:** No early strength
- **Why price:** Wait for a supported pullback with >=2R to observed resistance and <=8% stop distance
- **Entry:** 156.5–157.5
- **Invalidation:** Exit on loss of 145.5; thesis failure also invalidates
- **Add trigger:** Close above 169.5 with volume >=1.2x prior median; thesis still supported and recheck R/R
- **Main counter-evidence:** Unverified
- **Main risk:** Unverified causal interpretation
- **What would make us wrong:** A price-only story or weak company transmission

### 159. 1503.TW 士電 — WAIT

- **WHY:** WHY unresolved: obtain company evidence before taking risk
- **Driver:** Grid modernization / transformer and cable capex
- **Driver state:** UNVERIFIED
- **Company transmission:** Unverified
- **International confirmation:** Unverified — same-driver evidence required
- **Relative:** UNVERIFIED
- **Regime:** UNKNOWN
- **Technical state:** No early strength
- **Why price:** Wait for a supported pullback with >=2R to observed resistance and <=8% stop distance
- **Entry:** 203.5–202
- **Invalidation:** Exit on loss of 196; thesis failure also invalidates
- **Add trigger:** Close above 217 with volume >=1.2x prior median; thesis still supported and recheck R/R
- **Main counter-evidence:** Unverified
- **Main risk:** Unverified causal interpretation
- **What would make us wrong:** A price-only story or weak company transmission

### 160. 6285.TW 啟碁 — WAIT

- **WHY:** WHY unresolved: obtain company evidence before taking risk
- **Driver:** 800G/1.6T AI networking upgrade
- **Driver state:** UNVERIFIED
- **Company transmission:** Unverified
- **International confirmation:** Unverified — same-driver evidence required
- **Relative:** UNVERIFIED
- **Regime:** UNKNOWN
- **Technical state:** No early strength
- **Why price:** Wait for a supported pullback with >=2R to observed resistance and <=8% stop distance
- **Entry:** Unavailable
- **Invalidation:** Exit on loss of 223.5; thesis failure also invalidates
- **Add trigger:** Close above 257.5 with volume >=1.2x prior median; thesis still supported and recheck R/R
- **Main counter-evidence:** Unverified
- **Main risk:** Unverified causal interpretation
- **What would make us wrong:** A price-only story or weak company transmission

### 161. 3596.TW 智易 — WAIT

- **WHY:** WHY unresolved: obtain company evidence before taking risk
- **Driver:** 800G/1.6T AI networking upgrade
- **Driver state:** UNVERIFIED
- **Company transmission:** Unverified
- **International confirmation:** Unverified — same-driver evidence required
- **Relative:** UNVERIFIED
- **Regime:** UNKNOWN
- **Technical state:** No early strength
- **Why price:** Wait for a supported pullback with >=2R to observed resistance and <=8% stop distance
- **Entry:** Unavailable
- **Invalidation:** Exit on loss of 153.5; thesis failure also invalidates
- **Add trigger:** Close above 169 with volume >=1.2x prior median; thesis still supported and recheck R/R
- **Main counter-evidence:** Unverified
- **Main risk:** Unverified causal interpretation
- **What would make us wrong:** A price-only story or weak company transmission

### 162. 5388.TW 中磊 — WAIT

- **WHY:** WHY unresolved: obtain company evidence before taking risk
- **Driver:** 800G/1.6T AI networking upgrade
- **Driver state:** UNVERIFIED
- **Company transmission:** Unverified
- **International confirmation:** Unverified — same-driver evidence required
- **Relative:** UNVERIFIED
- **Regime:** UNKNOWN
- **Technical state:** No early strength
- **Why price:** Wait for a supported pullback with >=2R to observed resistance and <=8% stop distance
- **Entry:** 72.1–71.6
- **Invalidation:** Exit on loss of 70.6; thesis failure also invalidates
- **Add trigger:** Close above 81.6 with volume >=1.2x prior median; thesis still supported and recheck R/R
- **Main counter-evidence:** Unverified
- **Main risk:** Unverified causal interpretation
- **What would make us wrong:** A price-only story or weak company transmission

### 163. 2327.TW 國巨* — WAIT

- **WHY:** WHY unresolved: obtain company evidence before taking risk
- **Driver:** MLCC demand / pricing cycle
- **Driver state:** UNVERIFIED
- **Company transmission:** Unverified
- **International confirmation:** Unverified — same-driver evidence required
- **Relative:** UNVERIFIED
- **Regime:** UNKNOWN
- **Technical state:** No early strength
- **Why price:** Wait for a supported pullback with >=2R to observed resistance and <=8% stop distance
- **Entry:** 538–544
- **Invalidation:** Exit on loss of 502; thesis failure also invalidates
- **Add trigger:** Close above 677 with volume >=1.2x prior median; thesis still supported and recheck R/R
- **Main counter-evidence:** Unverified
- **Main risk:** Unverified causal interpretation
- **What would make us wrong:** A price-only story or weak company transmission

### 164. 2308.TW 台達電 — WAIT

- **WHY:** WHY unresolved: obtain company evidence before taking risk
- **Driver:** Power equipment / electronics capex
- **Driver state:** UNVERIFIED
- **Company transmission:** Unverified
- **International confirmation:** Unverified — same-driver evidence required
- **Relative:** UNVERIFIED
- **Regime:** UNKNOWN
- **Technical state:** No early strength
- **Why price:** Wait for a supported pullback with >=2R to observed resistance and <=8% stop distance
- **Entry:** Unavailable
- **Invalidation:** Exit on loss of 1650; thesis failure also invalidates
- **Add trigger:** Close above 2030 with volume >=1.2x prior median; thesis still supported and recheck R/R
- **Main counter-evidence:** Unverified
- **Main risk:** Unverified causal interpretation
- **What would make us wrong:** A price-only story or weak company transmission

### 165. 6446.TW 藥華藥 — WAIT

- **WHY:** WHY unresolved: obtain company evidence before taking risk
- **Driver:** Biopharma risk appetite / clinical-commercial cycle
- **Driver state:** UNVERIFIED
- **Company transmission:** Unverified
- **International confirmation:** Unverified — same-driver evidence required
- **Relative:** UNVERIFIED
- **Regime:** UNKNOWN
- **Technical state:** No early strength
- **Why price:** Wait for a supported pullback with >=2R to observed resistance and <=8% stop distance
- **Entry:** 1345–1280
- **Invalidation:** Exit on loss of 1255; thesis failure also invalidates
- **Add trigger:** Close above 1670 with volume >=1.2x prior median; thesis still supported and recheck R/R
- **Main counter-evidence:** Unverified
- **Main risk:** Unverified causal interpretation
- **What would make us wrong:** A price-only story or weak company transmission

### 166. 1605.TW 華新 — WAIT

- **WHY:** WHY unresolved: obtain company evidence before taking risk
- **Driver:** Copper commodity price / trade-inventory cycle
- **Driver state:** UNVERIFIED
- **Company transmission:** Unverified
- **International confirmation:** Unverified — same-driver evidence required
- **Relative:** UNVERIFIED
- **Regime:** UNKNOWN
- **Technical state:** No early strength
- **Why price:** Wait for a supported pullback with >=2R to observed resistance and <=8% stop distance
- **Entry:** 38.25–37.4
- **Invalidation:** Exit on loss of 37; thesis failure also invalidates
- **Add trigger:** Close above 40.85 with volume >=1.2x prior median; thesis still supported and recheck R/R
- **Main counter-evidence:** Unverified
- **Main risk:** Unverified causal interpretation
- **What would make us wrong:** A price-only story or weak company transmission

### 167. 4938.TW 和碩 — WAIT

- **WHY:** WHY unresolved: obtain company evidence before taking risk
- **Driver:** Consumer electronics demand cycle
- **Driver state:** UNVERIFIED
- **Company transmission:** Unverified
- **International confirmation:** Unverified — same-driver evidence required
- **Relative:** UNVERIFIED
- **Regime:** UNKNOWN
- **Technical state:** No early strength
- **Why price:** Wait for a supported pullback with >=2R to observed resistance and <=8% stop distance
- **Entry:** 90.4–91
- **Invalidation:** Exit on loss of 87; thesis failure also invalidates
- **Add trigger:** Close above 97.7 with volume >=1.2x prior median; thesis still supported and recheck R/R
- **Main counter-evidence:** Unverified
- **Main risk:** Unverified causal interpretation
- **What would make us wrong:** A price-only story or weak company transmission

### 168. 3704.TW 合勤控 — WAIT

- **WHY:** WHY unresolved: obtain company evidence before taking risk
- **Driver:** 800G/1.6T AI networking upgrade
- **Driver state:** UNVERIFIED
- **Company transmission:** Unverified
- **International confirmation:** Unverified — same-driver evidence required
- **Relative:** UNVERIFIED
- **Regime:** UNKNOWN
- **Technical state:** No early strength
- **Why price:** Wait for a supported pullback with >=2R to observed resistance and <=8% stop distance
- **Entry:** 40.2–40.5
- **Invalidation:** Exit on loss of 38.55; thesis failure also invalidates
- **Add trigger:** Close above 45.25 with volume >=1.2x prior median; thesis still supported and recheck R/R
- **Main counter-evidence:** Unverified
- **Main risk:** Unverified causal interpretation
- **What would make us wrong:** A price-only story or weak company transmission

### 169. 6223.TWO 旺矽 — WAIT

- **WHY:** WHY unresolved: obtain company evidence before taking risk
- **Driver:** Advanced packaging / test capex
- **Driver state:** UNVERIFIED
- **Company transmission:** Unverified
- **International confirmation:** Unverified — same-driver evidence required
- **Relative:** UNVERIFIED
- **Regime:** UNKNOWN
- **Technical state:** No early strength
- **Why price:** Wait for a supported pullback with >=2R to observed resistance and <=8% stop distance
- **Entry:** 5535–5620
- **Invalidation:** Exit on loss of 4780; thesis failure also invalidates
- **Add trigger:** Close above 6720 with volume >=1.2x prior median; thesis still supported and recheck R/R
- **Main counter-evidence:** Unverified
- **Main risk:** Unverified causal interpretation
- **What would make us wrong:** A price-only story or weak company transmission

### 170. 6472.TW 保瑞 — WAIT

- **WHY:** WHY unresolved: obtain company evidence before taking risk
- **Driver:** Biopharma risk appetite / clinical-commercial cycle
- **Driver state:** UNVERIFIED
- **Company transmission:** Unverified
- **International confirmation:** Unverified — same-driver evidence required
- **Relative:** UNVERIFIED
- **Regime:** UNKNOWN
- **Technical state:** No early strength
- **Why price:** Wait for a supported pullback with >=2R to observed resistance and <=8% stop distance
- **Entry:** Unavailable
- **Invalidation:** Exit on loss of 400; thesis failure also invalidates
- **Add trigger:** Close above 479.5 with volume >=1.2x prior median; thesis still supported and recheck R/R
- **Main counter-evidence:** Unverified
- **Main risk:** Unverified causal interpretation
- **What would make us wrong:** A price-only story or weak company transmission

### 171. 6140.TWO 訊達 — WAIT

- **WHY:** WHY unresolved: obtain company evidence before taking risk
- **Driver:** Enterprise cybersecurity spending
- **Driver state:** UNVERIFIED
- **Company transmission:** Unverified
- **International confirmation:** Unverified — same-driver evidence required
- **Relative:** UNVERIFIED
- **Regime:** UNKNOWN
- **Technical state:** No early strength
- **Why price:** Wait for a supported pullback with >=2R to observed resistance and <=8% stop distance
- **Entry:** 16.55–16.6
- **Invalidation:** Exit on loss of 15.95; thesis failure also invalidates
- **Add trigger:** Close above 17.9 with volume >=1.2x prior median; thesis still supported and recheck R/R
- **Main counter-evidence:** Unverified
- **Main risk:** Unverified causal interpretation
- **What would make us wrong:** A price-only story or weak company transmission

### 172. 3624.TWO 光頡 — WAIT

- **WHY:** WHY unresolved: obtain company evidence before taking risk
- **Driver:** Unmapped opportunity — research WHY
- **Driver state:** UNVERIFIED
- **Company transmission:** Unverified
- **International confirmation:** Unverified — same-driver evidence required
- **Relative:** UNVERIFIED
- **Regime:** UNKNOWN
- **Technical state:** Confirmed breakout
- **Why price:** Extended price: wait for a new base; do not chase
- **Entry:** 123–124.5
- **Invalidation:** Exit on loss of 83; thesis failure also invalidates
- **Add trigger:** Close above 115.5 with volume >=1.2x prior median; thesis still supported and recheck R/R
- **Main counter-evidence:** Unverified
- **Main risk:** Unverified causal interpretation
- **What would make us wrong:** A price-only story or weak company transmission

### 173. 2305.TW 全友 — WAIT

- **WHY:** WHY unresolved: obtain company evidence before taking risk
- **Driver:** Unmapped opportunity — research WHY
- **Driver state:** UNVERIFIED
- **Company transmission:** Unverified
- **International confirmation:** Unverified — same-driver evidence required
- **Relative:** UNVERIFIED
- **Regime:** UNKNOWN
- **Technical state:** Confirmed breakout
- **Why price:** Extended price: wait for a new base; do not chase
- **Entry:** 40.5–41
- **Invalidation:** Exit on loss of 28; thesis failure also invalidates
- **Add trigger:** Close above 39.1 with volume >=1.2x prior median; thesis still supported and recheck R/R
- **Main counter-evidence:** Unverified
- **Main risk:** Unverified causal interpretation
- **What would make us wrong:** A price-only story or weak company transmission

### 174. 3055.TW 蔚華科 — WAIT

- **WHY:** WHY unresolved: obtain company evidence before taking risk
- **Driver:** Unmapped opportunity — research WHY
- **Driver state:** UNVERIFIED
- **Company transmission:** Unverified
- **International confirmation:** Unverified — same-driver evidence required
- **Relative:** UNVERIFIED
- **Regime:** UNKNOWN
- **Technical state:** Early strength / support recovery
- **Why price:** Extended price: wait for a new base; do not chase
- **Entry:** 152–154.5
- **Invalidation:** Exit on loss of 114.5; thesis failure also invalidates
- **Add trigger:** Close above 167 with volume >=1.2x prior median; thesis still supported and recheck R/R
- **Main counter-evidence:** Unverified
- **Main risk:** Unverified causal interpretation
- **What would make us wrong:** A price-only story or weak company transmission

### 175. 6173.TWO 信昌電 — WAIT

- **WHY:** WHY unresolved: obtain company evidence before taking risk
- **Driver:** Unmapped opportunity — research WHY
- **Driver state:** UNVERIFIED
- **Company transmission:** Unverified
- **International confirmation:** Unverified — same-driver evidence required
- **Relative:** UNVERIFIED
- **Regime:** UNKNOWN
- **Technical state:** Early strength / support recovery
- **Why price:** Extended price: wait for a new base; do not chase
- **Entry:** 285.5–290
- **Invalidation:** Exit on loss of 224; thesis failure also invalidates
- **Add trigger:** Close above 300 with volume >=1.2x prior median; thesis still supported and recheck R/R
- **Main counter-evidence:** Unverified
- **Main risk:** Unverified causal interpretation
- **What would make us wrong:** A price-only story or weak company transmission

### 176. 3354.TWO 律勝 — WAIT

- **WHY:** WHY unresolved: obtain company evidence before taking risk
- **Driver:** Unmapped opportunity — research WHY
- **Driver state:** UNVERIFIED
- **Company transmission:** Unverified
- **International confirmation:** Unverified — same-driver evidence required
- **Relative:** UNVERIFIED
- **Regime:** UNKNOWN
- **Technical state:** Early strength / support recovery
- **Why price:** Extended price: wait for a new base; do not chase
- **Entry:** 29.4–29.85
- **Invalidation:** Exit on loss of 22.8; thesis failure also invalidates
- **Add trigger:** Close above 31.4 with volume >=1.2x prior median; thesis still supported and recheck R/R
- **Main counter-evidence:** Unverified
- **Main risk:** Unverified causal interpretation
- **What would make us wrong:** A price-only story or weak company transmission

### 177. 3229.TW 晟鈦 — WAIT

- **WHY:** WHY unresolved: obtain company evidence before taking risk
- **Driver:** Unmapped opportunity — research WHY
- **Driver state:** UNVERIFIED
- **Company transmission:** Unverified
- **International confirmation:** Unverified — same-driver evidence required
- **Relative:** UNVERIFIED
- **Regime:** UNKNOWN
- **Technical state:** Confirmed breakout
- **Why price:** Extended price: wait for a new base; do not chase
- **Entry:** 66.6–67.5
- **Invalidation:** Exit on loss of 47.4; thesis failure also invalidates
- **Add trigger:** Close above 66.8 with volume >=1.2x prior median; thesis still supported and recheck R/R
- **Main counter-evidence:** Unverified
- **Main risk:** Unverified causal interpretation
- **What would make us wrong:** A price-only story or weak company transmission

### 178. 7772.TWO 耀穎 — WAIT

- **WHY:** WHY unresolved: obtain company evidence before taking risk
- **Driver:** Unmapped opportunity — research WHY
- **Driver state:** UNVERIFIED
- **Company transmission:** Unverified
- **International confirmation:** Unverified — same-driver evidence required
- **Relative:** UNVERIFIED
- **Regime:** UNKNOWN
- **Technical state:** Early strength / support recovery
- **Why price:** Extended price: wait for a new base; do not chase
- **Entry:** 130.5–132
- **Invalidation:** Exit on loss of 85.7; thesis failure also invalidates
- **Add trigger:** Close above 146 with volume >=1.2x prior median; thesis still supported and recheck R/R
- **Main counter-evidence:** Unverified
- **Main risk:** Unverified causal interpretation
- **What would make us wrong:** A price-only story or weak company transmission

### 179. 5464.TWO 霖宏 — WAIT

- **WHY:** WHY unresolved: obtain company evidence before taking risk
- **Driver:** UNMAPPED / WHY?
- **Driver state:** UNVERIFIED
- **Company transmission:** Unverified
- **International confirmation:** Unverified — same-driver evidence required
- **Relative:** UNVERIFIED
- **Regime:** UNKNOWN
- **Technical state:** Early strength / support recovery
- **Why price:** Extended price: wait for a new base; do not chase
- **Entry:** 87.9–89
- **Invalidation:** Exit on loss of 66.7; thesis failure also invalidates
- **Add trigger:** Close above 92.3 with volume >=1.2x prior median; thesis still supported and recheck R/R
- **Main counter-evidence:** Unverified
- **Main risk:** Unverified causal interpretation
- **What would make us wrong:** A price-only story or weak company transmission

### 180. 1560.TW 中砂 — WAIT

- **WHY:** WHY unresolved: obtain company evidence before taking risk
- **Driver:** UNMAPPED / WHY?
- **Driver state:** UNVERIFIED
- **Company transmission:** Unverified
- **International confirmation:** Unverified — same-driver evidence required
- **Relative:** UNVERIFIED
- **Regime:** UNKNOWN
- **Technical state:** Confirmed breakout
- **Why price:** Extended price: wait for a new base; do not chase
- **Entry:** 783–790
- **Invalidation:** Exit on loss of 655; thesis failure also invalidates
- **Add trigger:** Close above 757 with volume >=1.2x prior median; thesis still supported and recheck R/R
- **Main counter-evidence:** Unverified
- **Main risk:** Unverified causal interpretation
- **What would make us wrong:** A price-only story or weak company transmission

### 181. 2338.TW 光罩 — WAIT

- **WHY:** WHY unresolved: obtain company evidence before taking risk
- **Driver:** UNMAPPED / WHY?
- **Driver state:** UNVERIFIED
- **Company transmission:** Unverified
- **International confirmation:** Unverified — same-driver evidence required
- **Relative:** UNVERIFIED
- **Regime:** UNKNOWN
- **Technical state:** Confirmed breakout
- **Why price:** Extended price: wait for a new base; do not chase
- **Entry:** 47.35–47.75
- **Invalidation:** Exit on loss of 37.3; thesis failure also invalidates
- **Add trigger:** Close above 46.1 with volume >=1.2x prior median; thesis still supported and recheck R/R
- **Main counter-evidence:** Unverified
- **Main risk:** Unverified causal interpretation
- **What would make us wrong:** A price-only story or weak company transmission

### 182. 5386.TWO 青雲 — WAIT

- **WHY:** WHY unresolved: obtain company evidence before taking risk
- **Driver:** UNMAPPED / WHY?
- **Driver state:** UNVERIFIED
- **Company transmission:** Unverified
- **International confirmation:** Unverified — same-driver evidence required
- **Relative:** UNVERIFIED
- **Regime:** UNKNOWN
- **Technical state:** Confirmed breakout
- **Why price:** Extended price: wait for a new base; do not chase
- **Entry:** 314.5–318
- **Invalidation:** Exit on loss of 233; thesis failure also invalidates
- **Add trigger:** Close above 315 with volume >=1.2x prior median; thesis still supported and recheck R/R
- **Main counter-evidence:** Unverified
- **Main risk:** Unverified causal interpretation
- **What would make us wrong:** A price-only story or weak company transmission

### 183. 6620.TWO 漢達 — WAIT

- **WHY:** WHY unresolved: obtain company evidence before taking risk
- **Driver:** UNMAPPED / WHY?
- **Driver state:** UNVERIFIED
- **Company transmission:** Unverified
- **International confirmation:** Unverified — same-driver evidence required
- **Relative:** UNVERIFIED
- **Regime:** UNKNOWN
- **Technical state:** Early strength / support recovery
- **Why price:** Extended price: wait for a new base; do not chase
- **Entry:** 122.5–124
- **Invalidation:** Exit on loss of 75.1; thesis failure also invalidates
- **Add trigger:** Close above 134 with volume >=1.2x prior median; thesis still supported and recheck R/R
- **Main counter-evidence:** Unverified
- **Main risk:** Unverified causal interpretation
- **What would make us wrong:** A price-only story or weak company transmission

### 184. 5880.TW 合庫金 — WAIT

- **WHY:** WHY unresolved: obtain company evidence before taking risk
- **Driver:** UNMAPPED / WHY?
- **Driver state:** UNVERIFIED
- **Company transmission:** Unverified
- **International confirmation:** Unverified — same-driver evidence required
- **Relative:** UNVERIFIED
- **Regime:** UNKNOWN
- **Technical state:** Confirmed breakout
- **Why price:** Extended price: wait for a new base; do not chase
- **Entry:** 27.1–27.2
- **Invalidation:** Exit on loss of 24.9; thesis failure also invalidates
- **Add trigger:** Close above 27 with volume >=1.2x prior median; thesis still supported and recheck R/R
- **Main counter-evidence:** Unverified
- **Main risk:** Unverified causal interpretation
- **What would make us wrong:** A price-only story or weak company transmission

### 185. 4556.TWO 旭然 — WAIT

- **WHY:** WHY unresolved: obtain company evidence before taking risk
- **Driver:** UNMAPPED / WHY?
- **Driver state:** UNVERIFIED
- **Company transmission:** Unverified
- **International confirmation:** Unverified — same-driver evidence required
- **Relative:** UNVERIFIED
- **Regime:** UNKNOWN
- **Technical state:** Confirmed breakout
- **Why price:** Extended price: wait for a new base; do not chase
- **Entry:** 93.4–94.5
- **Invalidation:** Exit on loss of 64.7; thesis failure also invalidates
- **Add trigger:** Close above 91.8 with volume >=1.2x prior median; thesis still supported and recheck R/R
- **Main counter-evidence:** Unverified
- **Main risk:** Unverified causal interpretation
- **What would make us wrong:** A price-only story or weak company transmission

### 186. 3374.TWO 精材 — WAIT

- **WHY:** WHY unresolved: obtain company evidence before taking risk
- **Driver:** UNMAPPED / WHY?
- **Driver state:** UNVERIFIED
- **Company transmission:** Unverified
- **International confirmation:** Unverified — same-driver evidence required
- **Relative:** UNVERIFIED
- **Regime:** UNKNOWN
- **Technical state:** Early strength / support recovery
- **Why price:** Extended price: wait for a new base; do not chase
- **Entry:** 457–463.5
- **Invalidation:** Exit on loss of 343; thesis failure also invalidates
- **Add trigger:** Close above 501 with volume >=1.2x prior median; thesis still supported and recheck R/R
- **Main counter-evidence:** Unverified
- **Main risk:** Unverified causal interpretation
- **What would make us wrong:** A price-only story or weak company transmission

### 187. 5314.TWO 世紀* — WAIT

- **WHY:** WHY unresolved: obtain company evidence before taking risk
- **Driver:** UNMAPPED / WHY?
- **Driver state:** UNVERIFIED
- **Company transmission:** Unverified
- **International confirmation:** Unverified — same-driver evidence required
- **Relative:** UNVERIFIED
- **Regime:** UNKNOWN
- **Technical state:** Early strength / support recovery
- **Why price:** Extended price: wait for a new base; do not chase
- **Entry:** 41.3–42.15
- **Invalidation:** Exit on loss of 29.9; thesis failure also invalidates
- **Add trigger:** Close above 47.7 with volume >=1.2x prior median; thesis still supported and recheck R/R
- **Main counter-evidence:** Unverified
- **Main risk:** Unverified causal interpretation
- **What would make us wrong:** A price-only story or weak company transmission

### 188. 3441.TWO 聯一光 — WAIT

- **WHY:** WHY unresolved: obtain company evidence before taking risk
- **Driver:** UNMAPPED / WHY?
- **Driver state:** UNVERIFIED
- **Company transmission:** Unverified
- **International confirmation:** Unverified — same-driver evidence required
- **Relative:** UNVERIFIED
- **Regime:** UNKNOWN
- **Technical state:** Confirmed breakout
- **Why price:** Extended price: wait for a new base; do not chase
- **Entry:** 163–166
- **Invalidation:** Exit on loss of 120; thesis failure also invalidates
- **Add trigger:** Close above 160.5 with volume >=1.2x prior median; thesis still supported and recheck R/R
- **Main counter-evidence:** Unverified
- **Main risk:** Unverified causal interpretation
- **What would make us wrong:** A price-only story or weak company transmission

### 189. 3406.TW 玉晶光 — WAIT

- **WHY:** WHY unresolved: obtain company evidence before taking risk
- **Driver:** UNMAPPED / WHY?
- **Driver state:** UNVERIFIED
- **Company transmission:** Unverified
- **International confirmation:** Unverified — same-driver evidence required
- **Relative:** UNVERIFIED
- **Regime:** UNKNOWN
- **Technical state:** Early strength / support recovery
- **Why price:** Extended price: wait for a new base; do not chase
- **Entry:** 1040–1060
- **Invalidation:** Exit on loss of 809; thesis failure also invalidates
- **Add trigger:** Close above 1150 with volume >=1.2x prior median; thesis still supported and recheck R/R
- **Main counter-evidence:** Unverified
- **Main risk:** Unverified causal interpretation
- **What would make us wrong:** A price-only story or weak company transmission

### 190. 4908.TWO 前鼎 — WAIT

- **WHY:** WHY unresolved: obtain company evidence before taking risk
- **Driver:** UNMAPPED / WHY?
- **Driver state:** UNVERIFIED
- **Company transmission:** Unverified
- **International confirmation:** Unverified — same-driver evidence required
- **Relative:** UNVERIFIED
- **Regime:** UNKNOWN
- **Technical state:** Early strength / support recovery
- **Why price:** Extended price: wait for a new base; do not chase
- **Entry:** 233–237.5
- **Invalidation:** Exit on loss of 178; thesis failure also invalidates
- **Add trigger:** Close above 270 with volume >=1.2x prior median; thesis still supported and recheck R/R
- **Main counter-evidence:** Unverified
- **Main risk:** Unverified causal interpretation
- **What would make us wrong:** A price-only story or weak company transmission

### 191. 2221.TWO 大甲 — WAIT

- **WHY:** WHY unresolved: obtain company evidence before taking risk
- **Driver:** UNMAPPED / WHY?
- **Driver state:** UNVERIFIED
- **Company transmission:** Unverified
- **International confirmation:** Unverified — same-driver evidence required
- **Relative:** UNVERIFIED
- **Regime:** UNKNOWN
- **Technical state:** Early strength / support recovery
- **Why price:** Extended price: wait for a new base; do not chase
- **Entry:** 93.8–95.4
- **Invalidation:** Exit on loss of 70.2; thesis failure also invalidates
- **Add trigger:** Close above 100.2 with volume >=1.2x prior median; thesis still supported and recheck R/R
- **Main counter-evidence:** Unverified
- **Main risk:** Unverified causal interpretation
- **What would make us wrong:** A price-only story or weak company transmission

### 192. 6225.TW 天瀚 — WAIT

- **WHY:** WHY unresolved: obtain company evidence before taking risk
- **Driver:** UNMAPPED / WHY?
- **Driver state:** UNVERIFIED
- **Company transmission:** Unverified
- **International confirmation:** Unverified — same-driver evidence required
- **Relative:** UNVERIFIED
- **Regime:** UNKNOWN
- **Technical state:** Early strength / support recovery
- **Why price:** Extended price: wait for a new base; do not chase
- **Entry:** 73.6–74.9
- **Invalidation:** Exit on loss of 51.2; thesis failure also invalidates
- **Add trigger:** Close above 79.6 with volume >=1.2x prior median; thesis still supported and recheck R/R
- **Main counter-evidence:** Unverified
- **Main risk:** Unverified causal interpretation
- **What would make us wrong:** A price-only story or weak company transmission

### 193. 6933.TW AMAX-KY — WAIT

- **WHY:** WHY unresolved: obtain company evidence before taking risk
- **Driver:** UNMAPPED / WHY?
- **Driver state:** UNVERIFIED
- **Company transmission:** Unverified
- **International confirmation:** Unverified — same-driver evidence required
- **Relative:** UNVERIFIED
- **Regime:** UNKNOWN
- **Technical state:** No early strength
- **Why price:** Extended price: wait for a new base; do not chase
- **Entry:** 315–320
- **Invalidation:** Exit on loss of 250.5; thesis failure also invalidates
- **Add trigger:** Close above 359.5 with volume >=1.2x prior median; thesis still supported and recheck R/R
- **Main counter-evidence:** Unverified
- **Main risk:** Unverified causal interpretation
- **What would make us wrong:** A price-only story or weak company transmission

### 194. 2605.TW 新興 — PASS

- **WHY:** Official August 2026 revenue data states that revenue increased significantly year over year because freight rates remained high.
- **Driver:** Higher Capesize and broader dry-bulk freight rates increase vessel charter revenue and utilization for dry-bulk operators.
- **Driver state:** DEVELOPING
- **Company transmission:** 新興's official monthly revenue disclosure attributes its August revenue increase directly to freight rates remaining high, linking the measured company improvement to the dry-bulk freight mechanism.
- **International confirmation:** Unverified — same-driver evidence required
- **Relative:** TOGETHER
- **Regime:** NEUTRAL
- **Technical state:** Early strength / support recovery
- **Why price:** Observed 120-session resistance offers at least 2R before costs; support defines <=8% price risk
- **Entry:** 36.15–36.5
- **Invalidation:** Exit on loss of 34.05; thesis failure also invalidates
- **Add trigger:** Close above 39.6 with volume >=1.2x prior median; thesis still supported and recheck R/R
- **Main counter-evidence:** BIMCO warned that apparent 2026 dry-bulk strength was substantially artificial and that forced rerouting support could reverse, with a potential 2027 supply glut. This creates a material durability risk even though current freight-rate evidence is supportive.
- **Main risk:** Freight rates can normalize as rerouting effects fade or vessel supply expands. The company is not classified as rate-sensitive because this field concerns real interest yields, long-end Treasury yields, liquidity and financing costs rather than freight-rate exposure.
- **What would make us wrong:** A sustained decline in Capesize/Panamax rates accompanied by weaker vessel utilization or company revenue reverting toward prior-year levels would falsify the transmission.
- Evidence: August 2026 monthly revenue was TWD 507861 thousand, up 58.75% year over year and 6.04% month over month; the issuer remark attributes the increase to freight rates remaining high. [Official monthly revenue: 新興 11508](https://mopsfin.twse.com.tw/opendata/t187ap05_L.csv)

### 195. 8227.TWO 巨有科技 — PASS

- **WHY:** WHY unresolved: obtain company evidence before taking risk
- **Driver:** Unmapped opportunity — research WHY
- **Driver state:** UNVERIFIED
- **Company transmission:** Unverified
- **International confirmation:** Unverified — same-driver evidence required
- **Relative:** UNVERIFIED
- **Regime:** UNKNOWN
- **Technical state:** Confirmed breakout
- **Why price:** Extended price: wait for a new base; do not chase
- **Entry:** 236.5–239
- **Invalidation:** Exit on loss of 152; thesis failure also invalidates
- **Add trigger:** Close above 220.5 with volume >=1.2x prior median; thesis still supported and recheck R/R
- **Main counter-evidence:** Unverified
- **Main risk:** Unverified causal interpretation
- **What would make us wrong:** A price-only story or weak company transmission

### 196. 6872.TWO 浩宇生醫 — PASS

- **WHY:** WHY unresolved: obtain company evidence before taking risk
- **Driver:** Unmapped opportunity — research WHY
- **Driver state:** UNVERIFIED
- **Company transmission:** Unverified
- **International confirmation:** Unverified — same-driver evidence required
- **Relative:** UNVERIFIED
- **Regime:** UNKNOWN
- **Technical state:** Early strength / support recovery
- **Why price:** Extended price: wait for a new base; do not chase
- **Entry:** 24.55–24.85
- **Invalidation:** Exit on loss of 13.45; thesis failure also invalidates
- **Add trigger:** Close above 27.4 with volume >=1.2x prior median; thesis still supported and recheck R/R
- **Main counter-evidence:** Unverified
- **Main risk:** Unverified causal interpretation
- **What would make us wrong:** A price-only story or weak company transmission

### 197. 6538.TWO 倉和 — PASS

- **WHY:** WHY unresolved: obtain company evidence before taking risk
- **Driver:** UNMAPPED / WHY?
- **Driver state:** UNVERIFIED
- **Company transmission:** Unverified
- **International confirmation:** Unverified — same-driver evidence required
- **Relative:** UNVERIFIED
- **Regime:** UNKNOWN
- **Technical state:** Early strength / support recovery
- **Why price:** Extended price: wait for a new base; do not chase
- **Entry:** 362.5–368
- **Invalidation:** Exit on loss of 191.5; thesis failure also invalidates
- **Add trigger:** Close above 359 with volume >=1.2x prior median; thesis still supported and recheck R/R
- **Main counter-evidence:** Unverified
- **Main risk:** Unverified causal interpretation
- **What would make us wrong:** A price-only story or weak company transmission


- BUY / EARLY BUY are advisory views at the displayed entry zone, never brokerage orders.
- A gap outside the zone or new thesis counter-evidence requires reassessment before taking risk.
- Prices and R/R use sealed closed sessions; upside references are resistance or disclosed base-height scenarios, not return forecasts.
- Automatic order execution remains disabled. Frozen execution permissions are unchanged.
